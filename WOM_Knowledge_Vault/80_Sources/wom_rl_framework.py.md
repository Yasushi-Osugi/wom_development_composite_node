...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom_rl_framework.py) · [原文テキスト](../90_Raw/wom_rl_framework.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `SCState` | 29 | Week t における SC の snapshot。 |
| ClassDef | `Operator` | 53 | WOM Planning Operator の基底クラス |
| ClassDef | `NormalProdOperator` | 61 | 通常生産: 需要 + 繰越分だけ生産（標準Operator） |
| ClassDef | `PrebuildOperator` | 73 | 先行生産（PUSH Mode 4）: 能力フル稼働でバッファ積み上げ |
| ClassDef | `ConserveOperator` | 85 | 節約生産: 需要の 50% のみ生産（コスト削減フェーズ） |
| ClassDef | `EmergencyReplenishOperator` | 97 | 緊急補充: 在庫逼迫時に残業で能力×1.2 生産 |
| ClassDef | `StaircaseDownOperator` | 109 | 段階縮小生産（DBR Staircase）: 製品 EOL フェーズで能力を 2/3 に絞る |
| FunctionDef | `evaluate` | 125 | U(S) = w1 × service_rate  （サービス充足率: 売上・顧客満足） |
| ClassDef | `WOMRLAgent` | 148 | Q-learning を用いた WOM Planning Operator 選択エージェント。 |
| ClassDef | `WOMEnvironment` | 220 | WOM Flow Engine の簡易実装。 |
| FunctionDef | `train` | 282 | Q-learning エージェントを訓練する。 |
| FunctionDef | `run_episode` | 339 | 学習済みエージェントでの 1 シーズン計画を表示する |
| FunctionDef | `to_key` | 40 | Q-table のキー（状態を離散化） |
| FunctionDef | `apply` | 57 | docstringなし（下のコード参照） |
| FunctionDef | `apply` | 65 | docstringなし（下のコード参照） |
| FunctionDef | `apply` | 77 | docstringなし（下のコード参照） |
| FunctionDef | `apply` | 89 | docstringなし（下のコード参照） |
| FunctionDef | `apply` | 101 | docstringなし（下のコード参照） |
| FunctionDef | `apply` | 113 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 159 | docstringなし（下のコード参照） |
| FunctionDef | `_get_q` | 173 | docstringなし（下のコード参照） |
| FunctionDef | `choose_action` | 178 | ε-greedy: 確率 ε でランダム探索、1-ε で最良 Operator 選択 |
| FunctionDef | `learn` | 188 | Bellman 方程式で Q 値を更新する |
| FunctionDef | `best_operator` | 208 | 現在の最良 Operator 名を返す（可視化用） |
| FunctionDef | `__init__` | 232 | docstringなし（下のコード参照） |
| FunctionDef | `_generate_lifecycle_demand` | 237 | SmartXPro ライフサイクル需要生成 |
| FunctionDef | `reset` | 252 | エピソード開始: 初期状態を返す |
| FunctionDef | `step` | 262 | 1 週間進める。 |
