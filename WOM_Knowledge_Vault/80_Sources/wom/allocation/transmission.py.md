...
tags: [wom, code]
---
# wom/allocation/transmission.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/allocation/transmission.py) · [原文テキスト](../../../90_Raw/wom/allocation/transmission.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `Scenario` | 44 | 外部環境シナリオ θ のうち、単位 P&L に効く成分。 |
| ClassDef | `CostBlock` | 59 | 1 チャネルの原価ブロック（per lot・関税前）＋販売条件。 |
| FunctionDef | `rates` | 103 | Step 0: 通貨→JPY レート（base_currency=JPY 前提）。 |
| FunctionDef | `unit_pnl` | 108 | Step 1〜5: 1 チャネルの per-lot 損益（JPY 建て）。 |
| FunctionDef | `unit_pnl_at_quantity` | 131 | 配分量 qty のときの per-lot 損益（Phase 5・cliff型の数量依存関税）。 |
| FunctionDef | `tariff_at` | 91 | 配分量 qty のときに適用される関税率（Step 3 用）。 |
