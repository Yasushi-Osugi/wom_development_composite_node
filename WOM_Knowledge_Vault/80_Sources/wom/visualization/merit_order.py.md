...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/visualization/merit_order.py) · [原文テキスト](../../../90_Raw/wom/visualization/merit_order.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `SupplierInfo` | 22 | サプライヤー情報 |
| ClassDef | `MeritOrderAnalyzer` | 39 | サプライヤーのコスト階層を分析し、需要別の最適配分を計算するアナライザー |
| FunctionDef | `load_suppliers_from_csv` | 405 | CSVからサプライヤーデータを読み込み |
| FunctionDef | `__init__` | 50 | 初期化 |
| FunctionDef | `validate_suppliers` | 74 | サプライヤーデータの整合性をチェック |
| FunctionDef | `calculate_merit_order` | 154 | Merit Order曲線を計算し、需要配分を算出 |
| FunctionDef | `allocate_demand` | 240 | 需要をサプライヤーに配分（最小コスト戦略） |
| FunctionDef | `_filter_suppliers` | 315 | 品質・リードタイム制約に基づいてサプライヤーをフィルタリング |
| FunctionDef | `_calculate_cost_in_usd` | 334 | 為替レートを適用してUSD建てのコストを計算 |
| FunctionDef | `_build_merit_order` | 346 | Merit Order構造を生成 |
| FunctionDef | `export_to_json` | 366 | 結果をJSON形式で保存 |
