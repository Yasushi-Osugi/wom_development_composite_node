...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/visualization/pareto_front.py) · [原文テキスト](../../../90_Raw/wom/visualization/pareto_front.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `ParetoFrontAnalyzer` | 21 | 複数目標（Cost, Quality, Lead Time）の Pareto 最適解を計算 |
| FunctionDef | `__init__` | 48 | 初期化 |
| FunctionDef | `from_merit_order_results` | 72 | calculate_merit_order() の結果リストから、 |
| FunctionDef | `compute_allocation_objectives` | 104 | 1 配分案の目的関数値を計算 |
| FunctionDef | `compute_plan_objectives` | 132 | merit_order result 1件 → 目的関数値 |
| FunctionDef | `_objective_fn` | 159 | 現在の粒度に応じた目的関数を返す（内部ヘルパー） |
| FunctionDef | `_dominates` | 166 | a が b を Pareto 支配するか判定 |
| FunctionDef | `_rank_front` | 185 | 目的関数値のリストから Pareto Front を判定する（内部共通ロジック）。 |
| FunctionDef | `compute_pareto_front` | 220 | Pareto Front を計算（O(n²) 素朴実装、n=配分案数は通常10-100程度想定） |
| FunctionDef | `compute_all_solutions` | 258 | 全解（支配されているものも含む）を目的関数値つきで返す。 |
| FunctionDef | `compute_tradeoff_ratios` | 297 | Pareto Front 上でのトレードオフ比率を計算（cost 昇順の隣接ランク間） |
| FunctionDef | `export_to_json` | 356 | 結果を JSON で保存 |
