...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/visualization/regime_map.py) · [原文テキスト](../../../90_Raw/wom/visualization/regime_map.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `classify_demand_level` | 27 | fulfillment_rate に基づいて需要レベルを分類 |
| FunctionDef | `classify_supply_tightness` | 50 | 平均リードタイムに基づいて供給タイト度を分類 |
| ClassDef | `RegimeMapAnalyzer` | 103 | Merit Order 結果から Regime Map を分類し、推奨戦略を提示 |
| FunctionDef | `__init__` | 241 | 初期化 |
| FunctionDef | `classify_single_week` | 266 | 単一週のレジーム分類 |
| FunctionDef | `classify_horizon` | 331 | 複数週の Regime 遷移を分析 |
| FunctionDef | `get_strategy_actions` | 399 | Regime に対応する具体的なアクション提示 |
| FunctionDef | `export_to_json` | 438 | 結果を JSON で保存 |
