...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/model/sc_tree.py) · [原文テキスト](../../../90_Raw/wom/model/sc_tree.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `BridgeTransfer` | 58 | docstringなし（下のコード参照） |
| ClassDef | `SCTree` | 69 | docstringなし（下のコード参照） |
| FunctionDef | `build_demo_sc_tree` | 269 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 70 | docstringなし（下のコード参照） |
| FunctionDef | `register` | 89 | docstringなし（下のコード参照） |
| FunctionDef | `register_extra_mom` | 100 | docstringなし（下のコード参照） |
| FunctionDef | `init_all_psi` | 105 | docstringなし（下のコード参照） |
| FunctionDef | `products` | 113 | docstringなし（下のコード参照） |
| FunctionDef | `get_ot_root` | 116 | docstringなし（下のコード参照） |
| FunctionDef | `get_in_root` | 122 | docstringなし（下のコード参照） |
| FunctionDef | `get_in_roots` | 128 | docstringなし（下のコード参照） |
| FunctionDef | `num_weeks` | 133 | docstringなし（下のコード参照） |
| FunctionDef | `week_idx` | 136 | docstringなし（下のコード参照） |
| FunctionDef | `iter_backward` | 139 | docstringなし（下のコード参照） |
| FunctionDef | `iter_forward` | 145 | docstringなし（下のコード参照） |
| FunctionDef | `iter_all_nodes` | 151 | docstringなし（下のコード参照） |
| FunctionDef | `bridge_backward` | 162 | docstringなし（下のコード参照） |
| FunctionDef | `bridge_forward` | 172 | docstringなし（下のコード参照） |
| FunctionDef | `_iter_all_nodes_global` | 181 | docstringなし（下のコード参照） |
| FunctionDef | `walk_ancestor_chain` | 186 | Walk start_node.parent upward until stop_node (exclusive), collecting |
| FunctionDef | `summary` | 247 | docstringなし（下のコード参照） |
| FunctionDef | `__repr__` | 261 | docstringなし（下のコード参照） |
