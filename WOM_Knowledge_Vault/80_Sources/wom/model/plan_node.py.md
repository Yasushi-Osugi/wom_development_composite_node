...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/model/plan_node.py) · [原文テキスト](../../../90_Raw/wom/model/plan_node.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `PlanNode` | 78 | One node in the WOM Planning Layer tree. |
| FunctionDef | `ss_wks` | 121 | Safety stock additional offset weeks = ceil(ss_days / 7). |
| FunctionDef | `init_stock_wks` | 126 | X2: initial (warm-up) stock offset weeks = ceil(init_stock_days / 7). |
| FunctionDef | `init_psi` | 210 | Allocate PSI list space for all weeks. |
| FunctionDef | `week_idx` | 252 | Return the integer index for an ISO week label string. |
| FunctionDef | `qty_demand` | 260 | Number of lots in psi4demand[week][bucket]. |
| FunctionDef | `qty_supply` | 264 | Number of lots in psi4supply[week][bucket]. |
| FunctionDef | `add_lot_demand` | 272 | Append a lot-ID to psi4demand[week][bucket]. |
| FunctionDef | `add_lot_supply` | 276 | Append a lot-ID to psi4supply[week][bucket]. |
| FunctionDef | `pop_lot_demand` | 280 | Remove and return the last lot-ID from psi4demand[week][bucket]. |
| FunctionDef | `pop_lot_supply` | 284 | Remove and return the last lot-ID from psi4supply[week][bucket]. |
| FunctionDef | `move_lot_demand` | 288 | Move a specific lot-ID within psi4demand from one (week, bucket) |
| FunctionDef | `cap_hard` | 305 | Equipment / physical capacity ceiling for the week [lot units]. |
| FunctionDef | `cap_soft` | 309 | Operational plan capacity ceiling for the week [lot units]. |
| FunctionDef | `set_capacity` | 313 | docstringなし（下のコード参照） |
| FunctionDef | `set_operating_shifts` | 326 | Set the operating shift count for one week. |
| FunctionDef | `operating_shifts` | 332 | Return the operating shift count for the week (None = unset / always open). |
| FunctionDef | `is_open` | 336 | True if the node operates this week. Unset (None) => always open; |
| FunctionDef | `kitting_required` | 346 | Set of child node_names this node's kitting must see arrive before a |
| FunctionDef | `kitting_status` | 357 | Read-only judgement for one (week, lot_id) kitting entry. |
| FunctionDef | `apply_carry_over_demand` | 379 | Forward Planning: if S demand cannot be fulfilled by I+P this week, |
| FunctionDef | `is_leaf` | 407 | docstringなし（下のコード参照） |
| FunctionDef | `is_root` | 410 | docstringなし（下のコード参照） |
| FunctionDef | `walk_postorder` | 413 | Yield nodes in POST-ORDER (children before parent). |
| FunctionDef | `walk_preorder` | 424 | Yield nodes in PRE-ORDER (parent before children). |
| FunctionDef | `add_child` | 439 | Attach a child node and set its parent back-pointer. |
| FunctionDef | `psi_summary` | 444 | Return a dict summarising demand-side PSI quantities for one week. |
| FunctionDef | `__repr__` | 460 | docstringなし（下のコード参照） |
| FunctionDef | `_empty_buckets` | 223 | docstringなし（下のコード参照） |
