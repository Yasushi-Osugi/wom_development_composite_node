...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/model/lot_generator.py) · [原文テキスト](../../../90_Raw/wom/model/lot_generator.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `LotIDGenerator` | 36 | Stateful counter that mints unique lot IDs for one planning run. |
| ClassDef | `LotAssignmentResult` | 118 | Summary of a demand lot assignment pass. |
| FunctionDef | `_build_leaf_index` | 138 | Build a dict  (sku_id, region) → leaf_out PlanNode |
| FunctionDef | `_infer_region_from_node` | 158 | Extract region from a leaf_out PlanNode. |
| FunctionDef | `assign_demand_lots_from_df` | 183 | Convert demand quantities into lot-ID lists on leaf_out nodes. |
| FunctionDef | `assign_demand_lots_from_dict` | 264 | Dict-based variant of assign_demand_lots_from_df. |
| FunctionDef | `lots_to_qty` | 296 | Convert a list of lot IDs back to demand quantity. |
| FunctionDef | `qty_to_lot_count` | 301 | Convert a demand quantity to the number of lots (ceiling division). |
| FunctionDef | `__init__` | 47 | docstringなし（下のコード参照） |
| FunctionDef | `generate` | 51 | Mint `count` new lot IDs for (sku_id, region, week). |
| FunctionDef | `reset` | 84 | Clear all counters (call between planning runs). |
| FunctionDef | `parse` | 91 | Parse a lot ID back into (sku_id, region, week, seq). |
| FunctionDef | `get_lots` | 128 | docstringなし（下のコード参照） |
