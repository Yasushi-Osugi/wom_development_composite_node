...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/planning_debugger.py) · [原文テキスト](../../../90_Raw/wom/engine/planning_debugger.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `OperatorStep` | 34 | One planning operator in the sequence. |
| ClassDef | `PlanningDebugger` | 49 | Manages step-by-step execution of the WOM Planning Engine. |
| FunctionDef | `__init__` | 58 | docstringなし（下のコード参照） |
| FunctionDef | `total_steps` | 78 | docstringなし（下のコード参照） |
| FunctionDef | `steps` | 82 | docstringなし（下のコード参照） |
| FunctionDef | `initialize` | 85 | Set up the debugger. |
| FunctionDef | `step_forward` | 116 | Execute the next operator step and capture a snapshot. |
| FunctionDef | `run_all` | 135 | Execute all remaining steps. |
| FunctionDef | `is_at_start` | 141 | docstringなし（下のコード参照） |
| FunctionDef | `is_at_end` | 145 | docstringなし（下のコード参照） |
| FunctionDef | `get_snapshot` | 148 | Return the PSI snapshot for the state *after* step_idx. |
| FunctionDef | `get_psi_arrays` | 160 | Return [[S, CO, I, P] x n_weeks] for node_name at step_idx. |
| FunctionDef | `get_delta` | 179 | Compute before/after delta for node_name at step_idx. |
| FunctionDef | `all_node_names` | 254 | Return all unique node names across all products (insertion order). |
| FunctionDef | `get_node` | 267 | Return PlanNode matching product + node_name, or None. |
| FunctionDef | `_take_snapshot` | 278 | Capture current PSI lot-counts for all nodes (lightweight copy). |
| FunctionDef | `get_shortfall` | 321 | Return per-week shortage count for a PUSH decoupling node at step_idx. |
| FunctionDef | `_fmt_changes` | 223 | docstringなし（下のコード参照） |
