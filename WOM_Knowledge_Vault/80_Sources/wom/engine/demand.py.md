...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/demand.py) · [原文テキスト](../../../90_Raw/wom/engine/demand.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `DemandEngine` | 30 | Prepares demand arrays for each SKU×Region×Scenario. |
| FunctionDef | `__init__` | 41 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 49 | Build base demand arrays from the forecast DataFrame. |
| FunctionDef | `get_demand` | 87 | Return demand array (length = num_weeks) for a given scenario. |
| FunctionDef | `avg_weekly_demand` | 100 | Average weekly demand across non-zero forecast weeks. |
| FunctionDef | `keys` | 104 | All (sku_id, region) pairs with demand data. |
| FunctionDef | `coverage_report` | 108 | Return a DataFrame showing forecast coverage per SKU×Region. |
