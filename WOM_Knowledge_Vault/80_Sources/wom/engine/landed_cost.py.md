...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/landed_cost.py) · [原文テキスト](../../../90_Raw/wom/engine/landed_cost.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `RouteProfile` | 59 | Cost parameters for one (src_region, dst_region) lane. |
| ClassDef | `LandedCostScenario` | 72 | Named set of RouteProfiles. |
| ClassDef | `RouteAssignment` | 88 | Maps (sku_id, region) → (src_region, dst_region, hs_code, assembly_cost). |
| FunctionDef | `load_edge_cost_master` | 103 | Load edge_cost_master.csv into a dict of LandedCostScenario. |
| FunctionDef | `load_route_master` | 133 | Load route_master.csv into a list of RouteAssignment. |
| FunctionDef | `build_route_index` | 156 | Build (sku_id, region) → RouteAssignment lookup dict. |
| FunctionDef | `compute_landed_cost_kpi` | 167 | Compute landed-cost-adjusted KPIs for one LandedCostScenario. |
| FunctionDef | `_no_adjustment_row` | 292 | docstringなし（下のコード参照） |
| FunctionDef | `filter_scenario_by_sku` | 309 | Scope a LandedCostScenario down to only the routes used by sku_id. |
| FunctionDef | `compare_lc_scenarios` | 344 | Run compute_landed_cost_kpi for all LC scenarios and stack results. |
| FunctionDef | `build_lc_narrative` | 380 | Build Japanese executive summary of landed cost scenario comparison. |
| FunctionDef | `get_profile` | 77 | Return the first matching RouteProfile, or None. |
