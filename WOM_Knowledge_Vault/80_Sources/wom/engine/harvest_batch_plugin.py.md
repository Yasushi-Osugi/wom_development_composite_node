...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/harvest_batch_plugin.py) · [原文テキスト](../../../90_Raw/wom/engine/harvest_batch_plugin.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `HarvestBatchPlugin` | 37 | Re-batches leaf_in production JIT -> harvest-week bursts and |
| FunctionDef | `__init__` | 59 | docstringなし（下のコード参照） |
| FunctionDef | `on_post_backward` | 67 | docstringなし（下のコード参照） |
| FunctionDef | `_inject_opening_inv` | 124 | Walk up the inbound chain and compute opening_inv for each ancestor. |
