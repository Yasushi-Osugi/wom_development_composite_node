...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/lane_assignment.py) · [原文テキスト](../../../90_Raw/wom/engine/lane_assignment.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `LaneAssignment` | 73 | docstringなし（下のコード参照） |
| ClassDef | `LaneTable` | 84 | Lookup table with two-level fallback: |
| FunctionDef | `__init__` | 93 | docstringなし（下のコード参照） |
| FunctionDef | `resolve` | 104 | Resolve mom_node_id with fallback hierarchy: |
| FunctionDef | `get_mom_node_id` | 126 | Direct key lookup (no fallback). Use resolve() for new code. |
| FunctionDef | `is_empty` | 130 | docstringなし（下のコード参照） |
| FunctionDef | `leaf_keys_for_sku` | 133 | All leaf_node_name / region keys defined for a given SKU. |
| FunctionDef | `mom_nodes_for_sku` | 137 | Distinct MOM node_ids assigned for a given SKU. |
| FunctionDef | `from_csv` | 146 | Load from lane_assignment.csv. |
| FunctionDef | `empty` | 165 | docstringなし（下のコード参照） |
| FunctionDef | `__repr__` | 172 | docstringなし（下のコード参照） |
