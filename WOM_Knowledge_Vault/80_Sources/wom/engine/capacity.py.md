...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/capacity.py) · [原文テキスト](../../../90_Raw/wom/engine/capacity.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `CapacityEngine` | 26 | Per-scenario capacity tracker. |
| ClassDef | `CapacityState` | 81 | Mutable capacity state for ONE simulation run (one scenario). |
| FunctionDef | `__init__` | 37 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 43 | Build capacity arrays from the capacity plan DataFrame. |
| FunctionDef | `for_scenario` | 57 | Return a mutable CapacityState for a single simulation run. |
| FunctionDef | `available_capacity` | 65 | Raw capacity for a single week (before any consumption). |
| FunctionDef | `__init__` | 87 | docstringなし（下のコード参照） |
| FunctionDef | `available` | 99 | Remaining capacity in a given week after prior consumption. |
| FunctionDef | `consume` | 108 | Record consumption of *qty* units of capacity in *week_idx*. |
| FunctionDef | `utilisation_report` | 120 | DataFrame of capacity utilisation for all SKU×Region×Week. |
