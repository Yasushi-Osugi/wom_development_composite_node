...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/warmup.py) · [原文テキスト](../../../90_Raw/wom/engine/warmup.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_parse_week` | 54 | docstringなし（下のコード参照） |
| FunctionDef | `_fmt_week` | 61 | docstringなし（下のコード参照） |
| FunctionDef | `weeks_between` | 66 | [start, end_exclusive) の ISO 週ラベル（古い順）。年跨ぎ（W53）も正しく扱う。 |
| FunctionDef | `week_minus` | 76 | label の n 週前の ISO ラベル。 |
| FunctionDef | `resolve_effective_start` | 83 | D2 の導出。real_start は「最初の非ゼロ需要週」。 |
| FunctionDef | `_read_lines` | 95 | docstringなし（下のコード参照） |
| FunctionDef | `_parse` | 104 | docstringなし（下のコード参照） |
| FunctionDef | `_col` | 108 | docstringなし（下のコード参照） |
| FunctionDef | `read_planning_config` | 115 | planning_config.csv（key,value）→ (warmup_lt or None, planning_start)。 |
| FunctionDef | `read_cpu_size` | 134 | planning_config.csv（key,value）の cpu_size キーを読む（Request Letter A: |
| FunctionDef | `first_nonzero_demand_week` | 160 | demand_forecast.csv の「最初の“非ゼロ”需要週」（＝real_start）。全ゼロ/空なら None。 |
| FunctionDef | `_rebuild` | 182 | (new_content, warm_row_count) を返す。実データ行は verbatim 保持。 |
| FunctionDef | `_build_warm_lines` | 205 | D1 の埋め方でCSV種別ごとに助走行（文字列）を生成。決定的な並び順で byte-stable。 |
| FunctionDef | `materialize_warmup` | 262 | planning_config.csv（または明示引数）に基づき助走行を materialize する。 |
| FunctionDef | `format_summary` | 335 | サマリーの1行ブロック（stdout 用）。 |
