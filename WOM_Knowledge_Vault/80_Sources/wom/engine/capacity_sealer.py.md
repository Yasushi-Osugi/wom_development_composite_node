...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/capacity_sealer.py) · [原文テキスト](../../../90_Raw/wom/engine/capacity_sealer.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `CapacityEntry` | 49 | Capacity specification for one node in one week. |
| ClassDef | `CapacityProfile` | 71 | Collection of CapacityEntry records that can be applied to an SCTree. |
| FunctionDef | `apply_capacity_profile` | 111 | Write CapHard / CapSoft from a CapacityProfile onto the corresponding |
| FunctionDef | `build_mom_capacity_profile` | 165 | Build a CapacityProfile that applies uniform CapHard / CapSoft |
| ClassDef | `CapacityLoadSummary` | 214 | Per-node, per-week capacity utilisation report generated |
| FunctionDef | `build_capacity_load_report` | 230 | Scan all PlanNodes for a product and return capacity utilisation |
| FunctionDef | `load_capacity_dataframe` | 280 | Apply a `capacity_plan.csv` DataFrame to the SCTree's PlanNodes. |
| FunctionDef | `load_operating_calendar` | 376 | Apply an `operating_calendar.csv` DataFrame to the SCTree's PlanNodes as a |
| FunctionDef | `add` | 77 | Append one entry (returns self for chaining). |
| FunctionDef | `add_flat` | 91 | Apply the same CapHard/CapSoft to a list of weeks for one node. |
| FunctionDef | `_soft_of` | 321 | cap_soft 列があり有効値なら float、無ければ 0.0（=無制限、後方互換）。 |
