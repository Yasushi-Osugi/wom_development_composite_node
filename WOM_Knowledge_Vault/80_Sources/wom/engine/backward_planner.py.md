...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/backward_planner.py) · [原文テキスト](../../../90_Raw/wom/engine/backward_planner.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `BackwardPlanResult` | 90 | Summary of one backward planning pass for a single product. |
| ClassDef | `BackwardPlanner` | 141 | Runs the backward planning pass for one or all products in an SCTree. |
| FunctionDef | `record_past_due` | 118 | docstringなし（下のコード参照） |
| FunctionDef | `record_cap_soft_envelope` | 121 | Plan-time flag: production placed within cap_hard exceeds cap_soft |
| FunctionDef | `__str__` | 129 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 161 | docstringなし（下のコード参照） |
| FunctionDef | `run` | 183 | Execute the full backward planning pass for one product. |
| FunctionDef | `run_all` | 252 | Run backward planning for every product in the SCTree. |
| FunctionDef | `_build_closed_index` | 263 | Build node_name -> set(closed week_idx), unioning two sources: |
| FunctionDef | `_offset_week` | 284 | Compute the upstream week index by stepping back lt_wks weeks, |
| FunctionDef | `_ot_propagate` | 310 | For each lot in node.S[w]: |
| FunctionDef | `_in_propagate` | 363 | For each lot in node.S[w]: |
| FunctionDef | `_propagate_to_children` | 411 | Distribute one week's demand lots from `node` to its InBound children, |
| FunctionDef | `_apply_mom_cap_backward` | 484 | v1r0m3: Constrained Demand Allocation for MOM nodes. |
| FunctionDef | `_build_lot_leaf_index` | 589 | Build lot_id -> leaf_out PlanNode index for multi-MOM lane routing. |
| FunctionDef | `_record_summary` | 614 | docstringなし（下のコード参照） |
