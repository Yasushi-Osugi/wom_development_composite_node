...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/strategic_kpi.py) · [原文テキスト](../../../90_Raw/wom/engine/strategic_kpi.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `StrategicKPI` | 58 | Strategic-layer KPIs derived from a post-planning SCTree. |
| FunctionDef | `compute_strategic_kpi` | 186 | Compute Strategic KPIs from a post-planning SCTree. |
| FunctionDef | `status_fixed_cost_coverage` | 73 | Return 'OK', 'WARN', or 'ISSUE'. |
| FunctionDef | `status_production_leveling` | 82 | docstringなし（下のコード参照） |
| FunctionDef | `status_buffer_retention` | 90 | docstringなし（下のコード参照） |
| FunctionDef | `status_fill_rate` | 98 | docstringなし（下のコード参照） |
| FunctionDef | `status_cap_utilization` | 106 | docstringなし（下のコード参照） |
| FunctionDef | `overall_status` | 114 | docstringなし（下のコード参照） |
| FunctionDef | `to_narrative_ja` | 128 | Short Japanese summary of strategic KPI status. |
