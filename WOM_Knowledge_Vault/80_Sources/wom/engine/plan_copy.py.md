...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/plan_copy.py) · [原文テキスト](../../../90_Raw/wom/engine/plan_copy.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `PlanCopyResult` | 49 | Summary of one demand→supply copy pass. |
| FunctionDef | `_copy_node` | 72 | Copy psi4demand[w][S] and psi4demand[w][P] into psi4supply for one node. |
| FunctionDef | `copy_demand_to_supply` | 95 | Copy the demand plan (psi4demand) to the supply layer (psi4supply) |
| FunctionDef | `copy_demand_to_supply_all` | 124 | Run copy_demand_to_supply for every product in the SCTree. |
| FunctionDef | `verify_copy_integrity` | 142 | Sanity-check that psi4supply matches psi4demand for S and P buckets, |
| FunctionDef | `__str__` | 58 | docstringなし（下のコード参照） |
