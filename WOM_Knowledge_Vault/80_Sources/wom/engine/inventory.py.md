...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/inventory.py) · [原文テキスト](../../../90_Raw/wom/engine/inventory.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_round_to_multiple` | 33 | Round *qty* up to the nearest *multiple* (1.0 = no rounding). |
| ClassDef | `InventorySimulator` | 40 | Runs the week-by-week inventory simulation for a single scenario. |
| FunctionDef | `__init__` | 48 | docstringなし（下のコード参照） |
| FunctionDef | `simulate` | 57 | Run simulation and return result DataFrame. |
