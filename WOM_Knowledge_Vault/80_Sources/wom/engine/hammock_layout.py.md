...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/hammock_layout.py) · [原文テキスト](../../../90_Raw/wom/engine/hammock_layout.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `compute_hammock_positions` | 94 | Compute (x, y) layout positions for all nodes of one product. |
| FunctionDef | `_reorder_outbound_by_lane` | 155 | Reorder OutBound DAD (DC) and leaf_out Y positions so that each DC's |
| FunctionDef | `compute_hammock_positions_all` | 217 | Compute positions for ALL products, stacked vertically. |
| FunctionDef | `build_lane_edges` | 249 | Derive physical logistics edges from lane_assignment.csv. |
| FunctionDef | `lane_colour_map` | 295 | Return {mom_node_id: colour} for the given lane edges. |
| FunctionDef | `build_hammock_graph` | 306 | Build a NetworkX DiGraph in hammock E2E layout for one product. |
| FunctionDef | `_add_edges_preorder` | 405 | Add parent→child topology edges for the tree rooted at root. |
| FunctionDef | `node_colour` | 416 | docstringなし（下のコード参照） |
| FunctionDef | `node_size` | 423 | docstringなし（下のコード参照） |
| FunctionDef | `_mom_y` | 191 | docstringなし（下のコード参照） |
