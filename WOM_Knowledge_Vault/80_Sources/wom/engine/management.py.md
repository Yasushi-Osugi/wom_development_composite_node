...
tags: [wom, code]
---
# wom/engine/management.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/management.py) · [原文テキスト](../../../90_Raw/wom/engine/management.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `ManagementIssue` | 28 | Confirmed management issue (degradation vs Base). |
| ClassDef | `ManagementRisk` | 39 | Early-warning / potential risk. |
| ClassDef | `ManagementAnalysisResult` | 49 | docstringなし（下のコード参照） |
| FunctionDef | `analyze_management_delta` | 75 | Compare scenario KPIs against base and produce issues/risks/narrative. |
| FunctionDef | `_build_narrative` | 273 | docstringなし（下のコード参照） |
| FunctionDef | `_pct_diff` | 320 | docstringなし（下のコード参照） |
| FunctionDef | `analyze_all_scenarios` | 330 | Analyze all non-Base scenarios and return a dict of results. |
| FunctionDef | `_row_to_money_dict` | 378 | docstringなし（下のコード参照） |
| FunctionDef | `_row_to_ops_dict` | 391 | docstringなし（下のコード参照） |
| FunctionDef | `_delta_pct` | 101 | docstringなし（下のコード参照） |
