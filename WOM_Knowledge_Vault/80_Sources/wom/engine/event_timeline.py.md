...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/event_timeline.py) · [原文テキスト](../../../90_Raw/wom/engine/event_timeline.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `NodeActivity` | 25 | Lot-count activity on one GUI node for one week. |
| ClassDef | `EdgeFlow` | 44 | Flow quantity on one directed edge for one week. |
| ClassDef | `WeekSnapshot` | 54 | Complete activity picture for one planning week. |
| FunctionDef | `build_event_timeline` | 77 | Build a list of WeekSnapshot from a fully-planned SCTree. |
| FunctionDef | `max_activity` | 180 | Return the maximum node.total across all weeks (for normalising sizes). |
| FunctionDef | `total` | 34 | docstringなし（下のコード参照） |
| FunctionDef | `flow` | 38 | Lots 'in motion' this week (S + P — ignoring I/CO for visual weight). |
| FunctionDef | `get_node` | 63 | docstringなし（下のコード参照） |
| FunctionDef | `max_flow` | 69 | docstringなし（下のコード参照） |
