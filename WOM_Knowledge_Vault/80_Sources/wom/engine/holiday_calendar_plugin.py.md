...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/engine/holiday_calendar_plugin.py) · [原文テキスト](../../../90_Raw/wom/engine/holiday_calendar_plugin.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `HolidayCalendarPlugin` | 17 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 22 | docstringなし（下のコード参照） |
| FunctionDef | `set_csv_path` | 27 | docstringなし（下のコード参照） |
| FunctionDef | `on_pre_plan` | 34 | docstringなし（下のコード参照） |
| FunctionDef | `_apply_supply_closure` | 77 | docstringなし（下のコード参照） |
| FunctionDef | `_apply_demand_multiplier` | 87 | docstringなし（下のコード参照） |
| FunctionDef | `on_post_backward` | 119 | Shift P-lots from explicitly-closed weeks to nearest open weeks. |
| FunctionDef | `_load_rules` | 223 | docstringなし（下のコード参照） |
| FunctionDef | `_infer_region` | 254 | docstringなし（下のコード参照） |
