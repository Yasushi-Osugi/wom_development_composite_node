...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/gui/app.py) · [原文テキスト](../../../90_Raw/wom/gui/app.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_445_ticks` | 83 | Return (tick_indices, tick_labels) for 4-4-5 quarterly X-axis display. |
| ClassDef | `LabeledEntry` | 124 | A label + entry widget pair. |
| ClassDef | `FileEntry` | 142 | Label + read-only entry + Browse button. |
| ClassDef | `ChartPanel` | 176 | Holds a matplotlib Figure with a toolbar and SKU/chart-type selectors. |
| ClassDef | `KPITablePanel` | 516 | Treeview-based KPI summary table. |
| ClassDef | `ManagementCockpitPanel` | 608 | Management Layer cockpit tab. |
| ClassDef | `PPCTabPanel` | 1391 | PPC Financial KPI tab — embeds PPCCockpitApp (B1 integration). |
| ClassDef | `PSIListPanel` | 1515 | Shows lot-ID based PSI data (psi4demand or psi4supply) for one PlanNode. |
| ClassDef | `SCNetworkPanel` | 1866 | 🌐 SC Network tab — split panel inspired by PySI GUI design. |
| ClassDef | `WorldMapPanel` | 3127 | 🗺 World Map tab — interactive SC node map using tkintermapview. |
| ClassDef | `DebugPanel` | 3714 | Step-by-step Planning Operation Debugger. |
| FunctionDef | `_detect_version_label` | 4361 | WOM のバージョン表記（例 'v1r2m3'）を自動検出する。 |
| ClassDef | `WOMApp` | 4390 | Top-level WOM application window. |
| FunctionDef | `launch` | 5648 | Entry point called by main.py. |
| FunctionDef | `__init__` | 126 | docstringなし（下のコード参照） |
| FunctionDef | `get` | 135 | docstringなし（下のコード参照） |
| FunctionDef | `set` | 138 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 144 | docstringなし（下のコード参照） |
| FunctionDef | `_browse` | 158 | docstringなし（下のコード参照） |
| FunctionDef | `get` | 165 | docstringなし（下のコード参照） |
| FunctionDef | `set` | 168 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 179 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 185 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 230 | docstringなし（下のコード参照） |
| FunctionDef | `refresh` | 241 | docstringなし（下のコード参照） |
| FunctionDef | `_filtered` | 272 | docstringなし（下のコード参照） |
| FunctionDef | `_ax_style` | 278 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_inventory` | 291 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_fill_rate` | 303 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_stockout` | 314 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_demand_supply` | 330 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_capacity` | 342 | docstringなし（下のコード参照） |
| FunctionDef | `_plot_cover` | 353 | docstringなし（下のコード参照） |
| FunctionDef | `load_sc_tree` | 365 | Store the post-planning SCTree for sc_tree-based charts. |
| FunctionDef | `_plot_buffer_stock` | 369 | Buffer Stock (MOM / Decoupling Point) — 産地集荷センター在庫推移 |
| FunctionDef | `_plot_harvest_input` | 440 | Harvest Input — 稲作田→産地集荷センター 週次収穫・出荷量 |
| FunctionDef | `__init__` | 519 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 523 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 573 | docstringなし（下のコード参照） |
| FunctionDef | `_apply_filter` | 580 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 626 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 632 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 866 | docstringなし（下のコード参照） |
| FunctionDef | `refresh_node_pl` | 876 | Public API — call after the PPC engine finishes a run (B2 thread), |
| FunctionDef | `_refresh_node_pl_table` | 884 | 拠点別P/L評価 — populate the Node P&L table from |
| FunctionDef | `_refresh_sku_filter` | 931 | (Re)populate the SKU dropdown from the latest summary_money. |
| FunctionDef | `_on_sku_filter_change` | 942 | docstringなし（下のコード参照） |
| FunctionDef | `_current_sku` | 949 | docstringなし（下のコード参照） |
| FunctionDef | `_get_filtered_kpi` | 953 | scenario_money_kpi for the current SKU filter. |
| FunctionDef | `_refresh_strategic_kpis` | 979 | Update the 5 Strategic KPI card widgets. |
| FunctionDef | `_refresh_lc_table` | 1016 | Update the Tariff & FX (Landed Cost) treeview and narrative. |
| FunctionDef | `_base_ccy` | 1081 | Return (code, symbol) of the PPC ledger's base currency for display |
| FunctionDef | `_ledger_lc_overrides` | 1101 | Phase 2 増分2 (v1r2m0): re-derive the Landed Cost table's |
| FunctionDef | `_ledger_pl_for_sku` | 1199 | Phase 2 (v1r2m0): single-source P&L top-line from the PPC event |
| FunctionDef | `_refresh_pl_table` | 1241 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_charts` | 1278 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_issue_selector` | 1344 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_issues` | 1359 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 1399 | docstringなし（下のコード参照） |
| FunctionDef | `_build_frame` | 1406 | docstringなし（下のコード参照） |
| FunctionDef | `_try_load` | 1448 | Load PPCCockpitApp from output_dir. Show placeholder if unavailable. |
| FunctionDef | `_show_placeholder` | 1500 | docstringなし（下のコード参照） |
| FunctionDef | `refresh` | 1505 | Public API — call after running the PPC engine. |
| FunctionDef | `__init__` | 1528 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 1536 | docstringなし（下のコード参照） |
| FunctionDef | `load_node` | 1645 | Load a PlanNode and display its PSI data. |
| FunctionDef | `_refresh` | 1652 | docstringなし（下のコード参照） |
| FunctionDef | `_on_select` | 1746 | Show lot IDs for the selected week row. |
| FunctionDef | `_draw_capacity_chart` | 1772 | Draw P-quantity bars with CapHard (red dashed) and CapSoft (orange |
| FunctionDef | `__init__` | 1884 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 1902 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 2040 | docstringなし（下のコード参照） |
| FunctionDef | `_build_graph` | 2060 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_graph` | 2109 | docstringなし（下のコード参照） |
| FunctionDef | `_on_node_click` | 2184 | docstringなし（下のコード参照） |
| FunctionDef | `_select_hammock_node` | 2211 | Handle click on a hammock-mode node. |
| FunctionDef | `_select_node` | 2235 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_right` | 2245 | docstringなし（下のコード参照） |
| FunctionDef | `_filter_sim` | 2253 | docstringなし（下のコード参照） |
| FunctionDef | `_ax_dark` | 2262 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_psi` | 2270 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_psi_from_plan_node` | 2336 | Draw PSI chart directly from node_obj.psi4supply (lot-level data). |
| FunctionDef | `_draw_cost_from_plan_node` | 2409 | Cost/Revenue chart from psi4supply S counts x per-node lot price. |
| FunctionDef | `_draw_cost` | 2537 | docstringなし（下のコード参照） |
| FunctionDef | `set_timeline` | 2604 | Called after planning completes with the built EventTimeline. |
| FunctionDef | `_anim_play` | 2618 | docstringなし（下のコード参照） |
| FunctionDef | `_anim_pause` | 2627 | docstringなし（下のコード参照） |
| FunctionDef | `_anim_stop` | 2632 | docstringなし（下のコード参照） |
| FunctionDef | `_on_speed_change` | 2645 | docstringなし（下のコード参照） |
| FunctionDef | `_anim_tick` | 2649 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_graph_animated` | 2663 | Redraw the network graph overlaid with one week's activity snapshot. |
| FunctionDef | `load_planning_tree` | 2783 | Load a post-planning SCTree. |
| FunctionDef | `_redraw_hammock` | 2812 | Build and draw the E2E hammock graph for the selected product. |
| FunctionDef | `_draw_hammock` | 2853 | Render the hammock graph (replaces _draw_graph in Planning mode). |
| FunctionDef | `_on_psi_node_select` | 3036 | Called when user picks a node from the PSI List combobox. |
| FunctionDef | `_try_update_psi_list` | 3047 |         When a network graph node is clicked, try to auto-select the |
| FunctionDef | `__init__` | 3135 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 3153 | docstringなし（下のコード参照） |
| FunctionDef | `_browse_node_master` | 3288 | docstringなし（下のコード参照） |
| FunctionDef | `load_default` | 3296 | Load a node master CSV path without user interaction. |
| FunctionDef | `_reload` | 3308 | docstringなし（下のコード参照） |
| FunctionDef | `_load_sc_tree_edges` | 3320 | Parse sc_tree_master.csv (if available) into real parent/child edges |
| FunctionDef | `_current_map_sku` | 3391 | docstringなし（下のコード参照） |
| FunctionDef | `_on_map_sku_change` | 3395 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_nodes` | 3400 | docstringなし（下のコード参照） |
| FunctionDef | `_fit_to_nodes` | 3501 | Fit the map view to the bounding box of the currently-drawn |
| FunctionDef | `_on_marker_click` | 3542 | Show node info in the right panel. |
| FunctionDef | `set_timeline` | 3563 | Enable lot-flow animation once planning is complete. |
| FunctionDef | `_anim_play` | 3574 | docstringなし（下のコード参照） |
| FunctionDef | `_anim_pause` | 3583 | docstringなし（下のコード参照） |
| FunctionDef | `_anim_stop` | 3588 | docstringなし（下のコード参照） |
| FunctionDef | `_map_tick` | 3601 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_animated_paths` | 3614 | Redraw SC paths with widths proportional to this week's lot flows. |
| FunctionDef | `__init__` | 3737 | docstringなし（下のコード参照） |
| FunctionDef | `_build_ui` | 3749 | docstringなし（下のコード参照） |
| FunctionDef | `_on_init` | 3886 | Build planning context and initialize debugger (runs in thread). |
| FunctionDef | `_init_worker` | 3893 | docstringなし（下のコード参照） |
| FunctionDef | `_on_init_done` | 3902 | docstringなし（下のコード参照） |
| FunctionDef | `_on_init_error` | 4039 | docstringなし（下のコード参照） |
| FunctionDef | `_on_reset` | 4043 | Re-initialize from scratch (re-reads all CSVs). |
| FunctionDef | `_on_next` | 4052 | docstringなし（下のコード参照） |
| FunctionDef | `_on_prev` | 4066 | Move back to view the previous step's snapshot (no re-execution). |
| FunctionDef | `_on_run_all` | 4077 | docstringなし（下のコード参照） |
| FunctionDef | `_on_product_change` | 4088 | docstringなし（下のコード参照） |
| FunctionDef | `_on_node_change` | 4092 | docstringなし（下のコード参照） |
| FunctionDef | `_on_step_done` | 4095 | docstringなし（下のコード参照） |
| FunctionDef | `_update_product_menu` | 4117 | docstringなし（下のコード参照） |
| FunctionDef | `_update_node_menu` | 4127 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_op_list` | 4144 | docstringなし（下のコード参照） |
| FunctionDef | `_set_buttons_enabled` | 4167 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_empty` | 4175 | docstringなし（下のコード参照） |
| FunctionDef | `_refresh_charts` | 4190 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_psi_subplot` | 4246 | Draw a single PSI chart (P/S/CO bars + I line + Capacity Line) on ax. |
| FunctionDef | `_set_delta_text` | 4351 | docstringなし（下のコード参照） |
| FunctionDef | `_strip` | 4368 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 4393 | docstringなし（下のコード参照） |
| FunctionDef | `_build_ui` | 4419 | docstringなし（下のコード参照） |
| FunctionDef | `_status` | 4476 | Update the status bar text. |
| FunctionDef | `_build_left_panel` | 4480 | docstringなし（下のコード参照） |
| FunctionDef | `_build_plugin_panel` | 4639 | Build the Plugin ON/OFF checklist below the progress bar. |
| FunctionDef | `_build_right_panel` | 4667 | docstringなし（下のコード参照） |
| FunctionDef | `_build_risk_tab` | 4712 | docstringなし（下のコード参照） |
| FunctionDef | `_build_delta_tab` | 4724 | docstringなし（下のコード参照） |
| FunctionDef | `_try_load_sample_paths` | 4734 | docstringなし（下のコード参照） |
| FunctionDef | `_load_model_folder` | 4760 | Open folder dialog; fill all 8 FileEntry fields from standard filenames. |
| FunctionDef | `_toggle_file_entries` | 4817 | Toggle collapse/expand of the 5 individual FileEntry widgets. |
| FunctionDef | `_auto_detect_planning_period` | 4828 | Read demand_forecast.csv and auto-set Start Week / # Weeks. |
| FunctionDef | `_build_config` | 4860 | docstringなし（下のコード参照） |
| FunctionDef | `_run_simulation` | 4881 | docstringなし（下のコード参照） |
| FunctionDef | `_simulate_thread` | 4891 | docstringなし（下のコード参照） |
| FunctionDef | `_on_simulation_done` | 4909 | docstringなし（下のコード参照） |
| FunctionDef | `_on_simulation_error` | 4972 | docstringなし（下のコード参照） |
| FunctionDef | `_load_risk_tab` | 4977 | docstringなし（下のコード参照） |
| FunctionDef | `_load_delta_tab` | 4995 | docstringなし（下のコード参照） |
| FunctionDef | `_build_planning_context` | 5041 | Build the shared planning context (sc_tree, weeks, HookBus, etc.) |
| FunctionDef | `_run_planning_engine` | 5198 | Build SCTree from input files (or demo data) and run the lot-based planning pipeline. |
| FunctionDef | `_planning_thread` | 5209 | docstringなし（下のコード参照） |
| FunctionDef | `_on_planning_done` | 5305 | docstringなし（下のコード参照） |
| FunctionDef | `_run_ppc_from_planning` | 5470 | B2: Trigger PPC simulation from PSI output in a background thread. |
| FunctionDef | `_on_ppc_done` | 5559 | Called on main thread after PPC engine completes. |
| FunctionDef | `_on_ppc_error` | 5578 | Called on main thread if PPC engine fails (non-fatal). |
| FunctionDef | `_on_planning_error` | 5586 | docstringなし（下のコード参照） |
| FunctionDef | `_export_csv` | 5597 | docstringなし（下のコード参照） |
| FunctionDef | `_export_excel` | 5616 | docstringなし（下のコード参照） |
| FunctionDef | `_status` | 5641 | docstringなし（下のコード参照） |
| FunctionDef | `_apply` | 996 | docstringなし（下のコード参照） |
| FunctionDef | `_worker` | 4083 | docstringなし（下のコード参照） |
| FunctionDef | `_left_frame_cfg` | 4448 | docstringなし（下のコード参照） |
| FunctionDef | `_left_canvas_cfg` | 4450 | docstringなし（下のコード参照） |
| FunctionDef | `_left_mousewheel` | 4452 | docstringなし（下のコード参照） |
| FunctionDef | `_ppc_thread` | 5538 | docstringなし（下のコード参照） |
| FunctionDef | `country_rate` | 1159 | docstringなし（下のコード参照） |
| FunctionDef | `scen_tariff` | 1170 | docstringなし（下のコード参照） |
| FunctionDef | `_push_step` | 3971 | docstringなし（下のコード参照） |
| FunctionDef | `_reset_sc_hint` | 5231 | docstringなし（下のコード参照） |
| FunctionDef | `_warn_demo_tree` | 5237 | docstringなし（下のコード参照） |
| FunctionDef | `_resolve_master` | 5400 | docstringなし（下のコード参照） |
