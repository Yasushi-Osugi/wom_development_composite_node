...
tags: [wom, code]
---
# wom/cockpit/s1_allocate.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/s1_allocate.py) · [原文テキスト](../../../90_Raw/wom/cockpit/s1_allocate.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `AllocationPanel` | 52 | S1 Allocate タブ。`load()` でモデルを読み、`view` を並べて描くだけ。 |
| FunctionDef | `__init__` | 55 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 77 | docstringなし（下のコード参照） |
| FunctionDef | `_on_panel_resize` | 258 | Phase 8-2a・D2: 「全市場の配分」控え欄の折返し幅を窓幅に追従させる。 |
| FunctionDef | `_on_browse` | 266 | docstringなし（下のコード参照） |
| FunctionDef | `_on_load` | 271 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 283 | モデルを読み、view を作って描く（公開 API・V2）。 |
| FunctionDef | `_reload_view` | 303 | docstringなし（下のコード参照） |
| FunctionDef | `_render` | 317 | docstringなし（下のコード参照） |
| FunctionDef | `_render_breadcrumb` | 367 | docstringなし（下のコード参照） |
| FunctionDef | `_reset_frame_size` | 383 | 子を空にした後もサイズが縮まない tkinter の挙動を回避する |
| FunctionDef | `_render_levels` | 410 | docstringなし（下のコード参照） |
| FunctionDef | `_render_children` | 427 | docstringなし（下のコード参照） |
| FunctionDef | `_render_node_info` | 474 | docstringなし（下のコード参照） |
| FunctionDef | `_render_plot` | 504 | docstringなし（下のコード参照） |
| FunctionDef | `_on_breadcrumb_click` | 585 | docstringなし（下のコード参照） |
| FunctionDef | `_on_up_click` | 591 | docstringなし（下のコード参照） |
| FunctionDef | `_on_child_click` | 596 | docstringなし（下のコード参照） |
| FunctionDef | `_on_apply_band` | 600 | 帯（台地の許容差）を手入力で上書きする（Phase 8-3a 追補）。 |
| FunctionDef | `commit` | 617 | 選んだ配分を保存する（公開 API・Phase 8-3a）。 |
| FunctionDef | `_on_merit_order` | 663 | docstringなし（下のコード参照） |
