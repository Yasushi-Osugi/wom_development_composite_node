...
tags: [wom, code]
---
# wom/cockpit/s3_view_model.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/s3_view_model.py) · [原文テキスト](../../../90_Raw/wom/cockpit/s3_view_model.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `run_s3` | 25 | S3 の手順4〜6を実行する（`tools/run_planning_loop.py` の手順4〜6と同じ）。 |
| FunctionDef | `_format_conclusion_ja` | 65 | N3: 結論行3行を組む。 |
| FunctionDef | `_capacity_nodes` | 101 | N5/N6: 能力を持つノードの一覧を、cap_hard 張り付き→cap_soft 超過の多い順に並べる。 |
| FunctionDef | `build_s3_view` | 147 | S3 に出す値をすべて作る。tkinter に依存しない。 |
| FunctionDef | `_pct_row` | 93 | docstringなし（下のコード参照） |
