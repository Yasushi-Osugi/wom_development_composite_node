...
tags: [wom, code]
---
# wom/cockpit/s1_view_model.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/s1_view_model.py) · [原文テキスト](../../../90_Raw/wom/cockpit/s1_view_model.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `format_market_name` | 56 | 市場名の表示用整形（Phase 8-2・C3）。'Retail_' 前置きを落とす。 |
| FunctionDef | `_load_scenario_row` | 78 | docstringなし（下のコード参照） |
| FunctionDef | `_scenario_blocks` | 114 | docstringなし（下のコード参照） |
| FunctionDef | `compute_reversal` | 125 | 現在のシナリオ fx から最も近い市場順位の反転点を返す（Phase 7a・A3.1 と同じ式）。 |
| FunctionDef | `_auto_skip_single_child` | 151 | 子1のノードは素通りする（V1.2）。子1のノードには全量がそのまま渡る。 |
| FunctionDef | `_walk_hierarchy` | 158 | `tree` を `node_path` に沿って降り、(現在ノード, そのノードの能力, 経路上のノード列)。 |
| FunctionDef | `_plot_kind` | 187 | docstringなし（下のコード参照） |
| FunctionDef | `_is_unallocated` | 195 | cap_lots がゼロ（または浮動小数の誤差程度）か（Phase 8-2・C2）。 |
| FunctionDef | `_unallocated_message_ja` | 204 | C2: 配分ゼロの枝に出す説明（child_x・図・台地サイズの代わりに1行）。 |
| FunctionDef | `_leaf_economics` | 222 | 葉ノード（末端市場）の単位経済（Phase 8-2・C4）。 |
| FunctionDef | `_format_market_share_ja` | 258 | 1市場分の『{名前} {比率}』表記（Phase 8-2a・D2）。 |
| FunctionDef | `_format_allocation_summary_ja` | 271 | 結論行に収める配分の要約（Phase 8-2・C3）。 |
| FunctionDef | `_format_full_allocation_ja` | 294 | 全市場の配分を省略なしで並べる（Phase 8-2・C3.3）。根拠パネルに全文で出す用。 |
| FunctionDef | `_bau_unit_spec` | 300 | `base` の桁から、共有する単位・小数桁数・単位記号を選ぶ |
| FunctionDef | `_format_money_pair_ja` | 312 | `base`（成り行きの利益）と `delta`（推奨配分との差）を**同じ単位・同じ |
| FunctionDef | `_format_bau_gap_ja` | 343 | 成り行き（H_willbe = `P_bau`）と推奨配分（H_tobe = `P_opt`）の差を1行にする |
| FunctionDef | `_bau_shares_ja` | 373 | 需要比例の BAU 配分（`x_bau`）を、いま見ているノードの子単位に射影する |
| FunctionDef | `_format_structural_gap_ja` | 397 | 構造由来の取りこぼしの文言（Phase 8-2・C6）。0 のとき「+0万」を出さない。 |
| FunctionDef | `_headline_lines` | 406 | docstringなし（下のコード参照） |
| FunctionDef | `_levels_list` | 422 | `levels` を値の降順で組む。P_greedy が P_grid を下回ったら highlight する |
| FunctionDef | `evaluate_allocation` | 443 | 選んだ配分1点を評価する（「⚑ この配分で計画する」用）。 |
| FunctionDef | `build_s1_view` | 466 | S1 に出す値をすべて作る。tkinter に依存しない。 |
