...
tags: [wom, code]
---
# wom/cockpit/navigator.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/navigator.py) · [原文テキスト](../../../90_Raw/wom/cockpit/navigator.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `PlanningNavigator` | 35 | ①。`render(current=, enabled=, passed=)` で描き直す。 |
| FunctionDef | `__init__` | 38 | docstringなし（下のコード参照） |
| FunctionDef | `render` | 45 | docstringなし（下のコード参照） |
