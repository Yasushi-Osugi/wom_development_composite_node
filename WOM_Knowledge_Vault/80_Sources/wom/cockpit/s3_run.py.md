...
tags: [wom, code]
---
# wom/cockpit/s3_run.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/s3_run.py) · [原文テキスト](../../../90_Raw/wom/cockpit/s3_run.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `RunPanel` | 42 | S3 Run タブ。`load()` で pre_plan を受け取り、`▶ 実行` で第2層・第3層を回す。 |
| FunctionDef | `__init__` | 45 | docstringなし（下のコード参照） |
| FunctionDef | `_build` | 62 | docstringなし（下のコード参照） |
| FunctionDef | `load` | 125 | S1 の pre_plan を受け取り、「未実行」状態で描く（公開 API）。 |
| FunctionDef | `commit` | 138 | 実行結果を保存する（公開 API・N7）。 |
| FunctionDef | `_on_run_clicked` | 159 | docstringなし（下のコード参照） |
| FunctionDef | `_poll_queue` | 184 | docstringなし（下のコード参照） |
| FunctionDef | `_render` | 206 | docstringなし（下のコード参照） |
| FunctionDef | `_render_nodes` | 232 | docstringなし（下のコード参照） |
| FunctionDef | `_on_node_click` | 254 | docstringなし（下のコード参照） |
| FunctionDef | `_render_result_info` | 261 | docstringなし（下のコード参照） |
| FunctionDef | `_render_plot` | 282 | docstringなし（下のコード参照） |
| FunctionDef | `_worker` | 173 | docstringなし（下のコード参照） |
