...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/data/schema.py) · [原文テキスト](../../../90_Raw/wom/data/schema.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `SKUMaster` | 10 | docstringなし（下のコード参照） |
| ClassDef | `DemandForecast` | 32 | docstringなし（下のコード参照） |
| ClassDef | `InventoryMaster` | 41 | docstringなし（下のコード参照） |
| ClassDef | `CapacityPlan` | 50 | docstringなし（下のコード参照） |
| ClassDef | `Cols` | 58 | Canonical DataFrame column names used throughout WOM. |
| FunctionDef | `key` | 27 | docstringなし（下のコード参照） |
