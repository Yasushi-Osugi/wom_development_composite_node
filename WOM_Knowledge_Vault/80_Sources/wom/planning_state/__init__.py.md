...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/planning_state/__init__.py) · [原文テキスト](../../../90_Raw/wom/planning_state/__init__.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_case_dir` | 58 | docstringなし（下のコード参照） |
| FunctionDef | `_next_allocation_id` | 62 | `out_dir/<case>/` の既存 `A\d+.json` から次の連番を決める（乱数・UUID 不使用）。 |
| FunctionDef | `new_state` | 76 | 計画案1件を新規作成する（`state="pre_plan"`）。 |
| FunctionDef | `save` | 110 | `state` を JSON として書き出し、書いたパスを返す。 |
| FunctionDef | `load` | 129 | docstringなし（下のコード参照） |
| FunctionDef | `list_states` | 135 | `case` の全計画案を `allocation_id` 昇順で返す（S5 の一覧用）。 |
| FunctionDef | `attach_placement` | 145 | 第2層（Backward配置）の結果を記録する（`state` は `pre_plan` のまま）。 |
| FunctionDef | `_read_csv` | 163 | docstringなし（下のコード参照） |
| FunctionDef | `_leaf_to_market` | 168 | leaf_out ノード名 -> market_group（`ga_market_aggregation.csv` 由来）。 |
| FunctionDef | `_leaf_currency` | 198 | leaf_out ノード名（= `ppc_market_price.csv` の `market_node`）-> 販売通貨。 |
| FunctionDef | `fx_effective_from_weekly` | 204 | 出荷数量加重平均の実効為替（Phase 7a・A2.4）。 |
| FunctionDef | `_evaluate_plan_at_fx` | 239 | A系統を `fx_effective` で再評価する（Phase 7a・A1 の `plan_at_realized_fx`）。 |
| FunctionDef | `attach_realized` | 265 | 第3層（Forward+PPC）の実績を `realized` に埋め、`state` を `feasible_plan` にする。 |
