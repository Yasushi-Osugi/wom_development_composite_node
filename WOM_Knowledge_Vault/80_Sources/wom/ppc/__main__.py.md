...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/ppc/__main__.py) · [原文テキスト](../../../90_Raw/wom/ppc/__main__.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `generate_sample_sales` | 46 | Generate synthetic sales_records using correct ISO 8601 week arithmetic. |
| FunctionDef | `_generate_model_sales` | 97 | Generate sample lots using actual products and channels from loaded rules. |
| FunctionDef | `_fmt` | 138 | docstringなし（下のコード参照） |
| FunctionDef | `print_kpi_banner` | 149 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 176 | docstringなし（下のコード参照） |
