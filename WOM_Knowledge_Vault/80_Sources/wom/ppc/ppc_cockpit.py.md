...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/ppc/ppc_cockpit.py) · [原文テキスト](../../../90_Raw/wom/ppc/ppc_cockpit.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_fmt` | 44 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_kpi_text` | 53 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_profit_zone` | 95 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_weekly_revenue` | 134 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_weekly_cost` | 160 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_margin_dist` | 182 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_fwd_bwd` | 219 | docstringなし（下のコード参照） |
| FunctionDef | `show_cockpit` | 253 | Load PPC output CSVs from `output_dir` and render the PPC Cockpit. |
