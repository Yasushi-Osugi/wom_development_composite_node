...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/ppc/ppc_backward.py) · [原文テキスト](../../../90_Raw/wom/ppc/ppc_backward.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_resolve_node` | 30 | docstringなし（下のコード参照） |
| FunctionDef | `_resolve_node_list` | 36 | Resolve dad_nodes_chain to an ordered list for a specific product/channel. |
| FunctionDef | `_node_costs_base` | 69 | Sum all node costs for (node_id, product) → base currency. |
| FunctionDef | `_edge_costs_base` | 95 | Sum all edge costs for edge_id → base currency. |
| FunctionDef | `_tariff_base` | 119 | Compute tariff on edge_id → base currency (0 if no rule). |
| FunctionDef | `run_backward_propagation` | 146 | Step 5: Lot-based backward requesting price propagation. |
