...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/ppc/ppc_cockpit_app.py) · [原文テキスト](../../../90_Raw/wom/ppc/ppc_cockpit_app.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_channel_short` | 80 | 'Retail_AMER' → 'AMER', 'JP_Channel' → 'JP' |
| FunctionDef | `_fmt` | 85 | docstringなし（下のコード参照） |
| FunctionDef | `_week_to_order` | 97 | '2026-W03' -> 202603 |
| FunctionDef | `_short_period` | 106 | Remove year prefix for axis labels: '2027-W03' → 'W03', 'M01' → 'M01' |
| FunctionDef | `_aggregate_weekly` | 114 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_kpi_text` | 156 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_profit_zone` | 246 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_weekly_revenue` | 298 | Revenue trend — dynamically use all channel nodes with revenue > 0. |
| FunctionDef | `_draw_weekly_cost` | 340 | docstringなし（下のコード参照） |
| FunctionDef | `_draw_margin_dist` | 362 | Lot Gross Margin boxplot — uses actual channels from data. |
| FunctionDef | `_draw_fwd_bwd` | 412 | Forward vs Backward vs Revenue — top 2 channels, dynamic. |
| FunctionDef | `_draw_waterfall` | 479 | Phase 3: Cost Waterfall — FOB/CIF/DAD Incoterm 区別付き。 |
| FunctionDef | `_count_trust_events` | 610 | Tally lot_reconciliation's pipe-separated 'trust_events_fired' column |
| FunctionDef | `_draw_trust_breakdown` | 631 | Trust Event Type Breakdown — horizontal bar chart of lot counts per |
| ClassDef | `PPCCockpitApp` | 678 | Interactive PPC Cockpit as a Tk Frame. |
| FunctionDef | `run_app` | 1019 | Launch the PPC Cockpit as a standalone Tk window. |
| FunctionDef | `_zone_sum` | 270 | docstringなし（下のコード参照） |
| FunctionDef | `_get_ch` | 444 | docstringなし（下のコード参照） |
| FunctionDef | `_avg` | 505 | docstringなし（下のコード参照） |
| FunctionDef | `_avg_phase` | 508 | docstringなし（下のコード参照） |
| FunctionDef | `__init__` | 684 | docstringなし（下のコード参照） |
| FunctionDef | `_load_data` | 692 | docstringなし（下のコード参照） |
| FunctionDef | `_build_ui` | 717 | docstringなし（下のコード参照） |
| FunctionDef | `_get_filtered_weeks` | 808 | docstringなし（下のコード参照） |
| FunctionDef | `_filter_data` | 817 | docstringなし（下のコード参照） |
| FunctionDef | `_redraw` | 840 | docstringなし（下のコード参照） |
| FunctionDef | `_on_canvas_click` | 905 | Click handler for the Trust Event Type Breakdown bar chart |
| FunctionDef | `_show_drilldown` | 923 | Opens a Toplevel window listing every Lot/Node/Week row (from the |
| FunctionDef | `_save_png` | 1005 | docstringなし（下のコード参照） |
