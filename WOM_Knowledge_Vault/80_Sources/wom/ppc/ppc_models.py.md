...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/ppc/ppc_models.py) · [原文テキスト](../../../90_Raw/wom/ppc/ppc_models.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `PPCEvent` | 60 | A single financial event in the PPC Simulation Engine. |
| ClassDef | `PPCTrustEvent` | 102 | Generated when reconciliation detects an unresolvable gap. |
| ClassDef | `LotCostAccumulator` | 122 | Mutable per-lot state accumulated during forward propagation. |
| ClassDef | `PPCSimulationResult` | 225 | Complete output of one PPC simulation run. |
| FunctionDef | `total_forward_cost_base` | 195 | docstringなし（下のコード参照） |
| FunctionDef | `gross_profit_base` | 212 | docstringなし（下のコード参照） |
| FunctionDef | `gross_margin_pct` | 215 | docstringなし（下のコード参照） |
