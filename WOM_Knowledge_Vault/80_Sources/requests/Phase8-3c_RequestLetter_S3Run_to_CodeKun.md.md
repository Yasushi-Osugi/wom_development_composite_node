...
tags: [wom, source]
---
# requests/Phase8-3c_RequestLetter_S3Run_to_CodeKun.md

原文資料（記載時点の状態を含む）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/requests/Phase8-3c_RequestLetter_S3Run_to_CodeKun.md) · [原文テキスト](../../90_Raw/requests/Phase8-3c_RequestLetter_S3Run_to_CodeKun.md.txt)
...
> **【訂正・実装後に追記】本節の「cap_hard 超過」は誤りだった。**
> `cap_hard` は Forward planner が封じる（seal）ので **P は cap_hard を超えない**。
> 正しくは**張り付き**（上限で抑え込まれ、需要が溢れた）。`cap_soft` は flag-only
> なので「超過」で正しい。この誤った語のまま実装された結果、結論行 4週 / ノード
> 一覧 計2週 という食い違いが出た（`p > cap_hard` が成立しないため）。正典は
> planner 自身の記録 `cap_hard_events` で、そこの `node_id` を捨てていたのが原因。
> `capacity_events` としてノード別に出す形に直した。設計書 rev.6 も訂正済み。
