...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/README.md) · [原文テキスト](../90_Raw/README.md.txt)
...
**N市場化（Phase 6）— 実装済み** — δ=0.05 の格子点数は市場数 N に対して `C(20+N-1, N-1)` で増え、平坦な grid scan は N=6（53,130点）前後が現実的な上限です。市場を3つ以下ずつのグループに束ねる**階層化単体格子**（`hierarchical_simplex`）なら、`oil-global-2027`（lot 単位で分離した `uom="KL"` の15市場）を **10ノード・1,050点**で扱えます（平坦全数は `C(34,14)` ≈ 13.9億点で原理的に走りません・削減比約132万倍）。その近似誤差は、格子を経由しない `true_continuous_optimum()` を基準にした `hierarchy_gap()` で金額として測れます（cap_wk により誤差0.5〜3%程度）。

- 6-1 `true_optimum` の実計算化 — **実装済み**（関税の崖の on/off を全列挙し、閾値を配分量の上下界として制約に持たせて厳密に解く）
- 6-2 次元の一般化（`MARKETS` / `simplex_grid` の N 次元化）— **実装済み**。市場の並びは `ga_market_aggregation.csv` から導出し、配分エンジン（利益地形・メリットオーダー・真の最適・レジーム地図）は N 市場で動きます（平坦格子は N=6・53,130点まで。N=7 以上は計算に入る前に停止）。直角三角図は単体の2次元射影のため N=3 専用のままで、N≥4 は 6-3 の階層ドリルダウン経由で見ます
- 6-3 階層化単体格子（`hierarchical_simplex`）— **実装済み**。`build_hierarchy()` が供給元 Mother Plant → 通貨圏 → 供給モードの順にグループ木を組み、`scan_hierarchical()` が案B（逐次確定型）で降りながらスキャンします。`oil-global-2027` の接続では、1 lot の物理単位が市場ごとに異なる（`KL` と、タンカー単位の `KL100KBBL`）ことが判明し、`derive_cost_blocks(uom=...)` で単位ごとに別の配分問題として分離しました（単位変換ではなく問題の分離。詳細は `docs/design/ask_global_allocation_spec.md` §2.6）
