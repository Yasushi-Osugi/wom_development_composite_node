...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/plot_allocation_merit_regime.py) · [原文テキスト](../../90_Raw/tools/plot_allocation_merit_regime.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_draw_block` | 60 | docstringなし（下のコード参照） |
| FunctionDef | `plot_allocation_merit_order` | 68 | メリットオーダー曲線（配分版）を描画し、保存先パスを返す。 |
| FunctionDef | `plot_allocation_merit_shift` | 197 | FX等の変化で市場順位が入れ替わる様子を、2本の階段で重ねる（配分版）。 |
| FunctionDef | `_axis_label` | 305 | docstringなし（下のコード参照） |
| FunctionDef | `plot_regime_map` | 320 | 外部環境パラメータ平面のレジーム地図を描画し、保存先パスを返す。 |
| FunctionDef | `_material_axis_values` | 417 | 基準値の -40%~+70% を13点で均等に刻む。 |
| FunctionDef | `_material_shock_value` | 430 | docstringなし（下のコード参照） |
| FunctionDef | `_build_arg_parser` | 434 | docstringなし（下のコード参照） |
| FunctionDef | `parse_args` | 447 | docstringなし（下のコード参照） |
| FunctionDef | `run` | 451 | 引数に従って4枚のPNGを生成し、生成したパスのリストを返す。 |
| FunctionDef | `main` | 521 | docstringなし（下のコード参照） |
| FunctionDef | `_step_arrays` | 208 | docstringなし（下のコード参照） |
