...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/plot_merit_order_suite.py) · [原文テキスト](../../90_Raw/tools/plot_merit_order_suite.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_horizon_required_qty` | 73 | 決定的な週次需要変動を返す（乱数不使用、Request Letter §C7）。 |
| FunctionDef | `_apply_shock` | 79 | 最安サプライヤーのコストを factor 倍にする決定的な擾乱 |
| FunctionDef | `_plan_scenarios` | 100 | データから自動導出した、汎用的な demand フィルタシナリオ（決定的）。 |
| FunctionDef | `_draw_block` | 139 | docstringなし（下のコード参照） |
| FunctionDef | `plot_merit_order_curve` | 146 | Merit Order 曲線を描画し、保存先パスを返す。 |
| FunctionDef | `plot_merit_order_shift` | 235 | 為替・関税の変化でサプライヤー順位が入れ替わる様子を、2本の階段で重ねる。 |
| FunctionDef | `_weeks_label` | 358 | 週数を単複を考慮した英語表記にする（例: 0 weeks / 1 week / 9 weeks） |
| FunctionDef | `plot_regime_matrix` | 363 | 3×3 の Regime Map ヒートマップを描画する。 |
| FunctionDef | `plot_regime_timeline` | 414 | Regime score の時系列 + 推奨戦略の帯を描画する。 |
| FunctionDef | `plot_pareto_scatter` | 475 | 目的空間の散布図（2パネル）を描画し、保存先パスを返す。 |
| FunctionDef | `build_supplier_share_axes` | 570 | 全配分案の和集合から Merit Order 順に上位 K 社の軸を決め、 |
| FunctionDef | `plot_parallel_coordinates` | 645 | 目的軸3本（+ サプライヤー share 軸）の平行座標図を描画する。 |
| FunctionDef | `_build_arg_parser` | 772 | docstringなし（下のコード参照） |
| FunctionDef | `parse_args` | 786 | docstringなし（下のコード参照） |
| FunctionDef | `run` | 790 | 引数に従って6枚のPNGを生成し、生成したパスのリストを返す。 |
| FunctionDef | `main` | 864 | docstringなし（下のコード参照） |
| FunctionDef | `_step_arrays` | 246 | docstringなし（下のコード参照） |
| FunctionDef | `_lambda_of` | 266 | docstringなし（下のコード参照） |
| FunctionDef | `_norm` | 656 | docstringなし（下のコード参照） |
