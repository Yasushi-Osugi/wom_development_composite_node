...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/gen_apparel_global_model.py) · [原文テキスト](../../90_Raw/tools/gen_apparel_global_model.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `product_id` | 248 | docstringなし（下のコード参照） |
| FunctionDef | `week_infos` | 255 | docstringなし（下のコード参照） |
| FunctionDef | `trim_weeks_to` | 265 | Trim the week list to stop right after end_label (inclusive). |
| FunctionDef | `demand_weight` | 275 | Baseline outside [start,end]; raised-sine bump 1.0 at mid-season. |
| FunctionDef | `ramp_factor` | 285 | Linear 0->1 ramp over the simulation's first `ramp_weeks` weeks |
| FunctionDef | `gen_sku_master` | 345 | docstringなし（下のコード参照） |
| FunctionDef | `gen_node_master` | 378 | docstringなし（下のコード参照） |
| FunctionDef | `gen_sc_tree_master` | 416 | docstringなし（下のコード参照） |
| FunctionDef | `gen_lane_assignment` | 452 | docstringなし（下のコード参照） |
| FunctionDef | `gen_route_master` | 463 | docstringなし（下のコード参照） |
| FunctionDef | `gen_edge_cost_master` | 475 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_tariff_rule` | 487 | Outbound tariff: last_dad(DC_market) -> channel(Retail_market). |
| FunctionDef | `gen_ppc_transfer_price_rule` | 501 | Only the TERMINAL mom (Garment) gets a transfer-price rule -- see |
| FunctionDef | `gen_node_cost_master` | 514 | Informational (Management-panel display) cost breakdown. |
| FunctionDef | `gen_ppc_edge_cost_rule` | 553 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_node_cost_rule` | 578 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_node_profit_zone` | 599 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_profit_zone_rule` | 616 | docstringなし（下のコード参照） |
| FunctionDef | `gen_holiday_calendar` | 629 | docstringなし（下のコード参照） |
| FunctionDef | `gen_inventory_master` | 647 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_supplier_cost` | 659 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_market_price` | 669 | docstringなし（下のコード参照） |
| FunctionDef | `gen_demand_forecast` | 680 | docstringなし（下のコード参照） |
| FunctionDef | `gen_capacity_plan` | 739 | Cover ALL weeks for every capacity-bearing node (not just demand |
| FunctionDef | `gen_fx_rate` | 769 | Base-case FX assumption for the live simulation. USD/JPY held flat |
| FunctionDef | `main` | 786 | docstringなし（下のコード参照） |
