...
tags: [wom, code]
---
# tools/proto_terrain2.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/proto_terrain2.py) · [原文テキスト](../../90_Raw/tools/proto_terrain2.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `rates` | 26 | docstringなし（下のコード参照） |
| FunctionDef | `unit` | 30 | docstringなし（下のコード参照） |
| FunctionDef | `evaluate` | 40 | docstringなし（下のコード参照） |
| FunctionDef | `grid` | 56 | docstringなし（下のコード参照） |
| FunctionDef | `analyze` | 64 | docstringなし（下のコード参照） |
