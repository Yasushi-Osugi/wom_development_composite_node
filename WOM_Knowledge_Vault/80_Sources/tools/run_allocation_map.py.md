...
tags: [wom, code]
---
# tools/run_allocation_map.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/run_allocation_map.py) · [原文テキスト](../../90_Raw/tools/run_allocation_map.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_rows` | 42 | docstringなし（下のコード参照） |
| FunctionDef | `load_scenarios` | 47 | ga_scenario_master.csv → シナリオごとの代表パラメータ。 |
| FunctionDef | `_blocks_for` | 108 | シナリオの関税率で CostBlock を上書き（tariff は Step3 で使用）。 |
| FunctionDef | `_fmt_x` | 132 | docstringなし（下のコード参照） |
| FunctionDef | `run` | 137 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 261 | docstringなし（下のコード参照） |
| FunctionDef | `_w` | 156 | docstringなし（下のコード参照） |
