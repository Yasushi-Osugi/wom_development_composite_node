...
tags: [wom, code]
---
# tools/download_worldmap_tiles.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/download_worldmap_tiles.py) · [原文テキスト](../../90_Raw/tools/download_worldmap_tiles.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_tile_xy` | 53 | 緯度経度 → タイル (x, y) 変換（Web Mercator） |
| FunctionDef | `_bbox_tile_range` | 62 | docstringなし（下のコード参照） |
| FunctionDef | `_init_db` | 84 | docstringなし（下のコード参照） |
| FunctionDef | `_download_region` | 105 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 150 | docstringなし（下のコード参照） |
