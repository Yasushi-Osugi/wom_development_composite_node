...
tags: [wom, code]
---
# tools/run_planning_loop.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/run_planning_loop.py) · [原文テキスト](../../90_Raw/tools/run_planning_loop.py.txt)
...
使い方（リポジトリ直下）:
  python -m tools.run_planning_loop --model-dir data/sample/soysauce-jpy-2027-alloc          --scenario s1_base --cap-wk 800 --allocation 0.10,0.45,0.45
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `compute_reversal` | 58 | 現在のシナリオ fx から最も近い市場順位の反転点を返す（Phase 7a・A3.1）。 |
| FunctionDef | `_scenario_blocks` | 85 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 100 | docstringなし（下のコード参照） |
| FunctionDef | `_id_from_path` | 236 | docstringなし（下のコード参照） |
| FunctionDef | `_format_conclusion` | 240 | 結論を日本語1行ブロックで組む（Phase 7a・A4）。 |
| FunctionDef | `pct_row` | 265 | docstringなし（下のコード参照） |
