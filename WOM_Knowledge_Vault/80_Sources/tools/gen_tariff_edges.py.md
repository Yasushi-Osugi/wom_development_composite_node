...
tags: [wom, code]
---
# tools/gen_tariff_edges.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/gen_tariff_edges.py) · [原文テキスト](../../90_Raw/tools/gen_tariff_edges.py.txt)
...
Usage:
    ...
    python -m tools.gen_tariff_edges --model-dir <dir> --scenario Base            --out ppc_tariff_rule.csv [--check]
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_read_csv` | 50 | docstringなし（下のコード参照） |
| FunctionDef | `_node_country_map` | 57 | (node_id, product_id) -> country, from ppc_node_profit_zone.csv. |
| FunctionDef | `_hs_code_map` | 66 | (sku_id, region) -> hs_code, from route_master.csv (best effort). |
| FunctionDef | `_load_canonical` | 75 | Load canonical edge_cost_master for one scenario. |
| FunctionDef | `_lookup_canonical` | 95 | Prefer an exact product row, fall back to the '*' wildcard row. |
| FunctionDef | `generate` | 101 | Return the list of generated ppc_tariff_rule rows (dicts). |
| FunctionDef | `_write` | 173 | docstringなし（下のコード参照） |
| FunctionDef | `_norm` | 181 | Normalise a numeric-ish string for comparison (0.10 == 0.1). |
| FunctionDef | `_check` | 190 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 214 | docstringなし（下のコード参照） |
| FunctionDef | `country_of` | 114 | docstringなし（下のコード参照） |
