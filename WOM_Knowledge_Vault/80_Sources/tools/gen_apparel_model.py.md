...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/gen_apparel_model.py) · [原文テキスト](../../90_Raw/tools/gen_apparel_model.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `sales_node` | 196 | docstringなし（下のコード参照） |
| FunctionDef | `product_id` | 200 | docstringなし（下のコード参照） |
| FunctionDef | `week_infos` | 207 | Return list of (label, iso_week) for num_weeks consecutive weeks. |
| FunctionDef | `season_weight` | 218 | Raised-sine bump: 0 at the season edges, 1 at mid-season. |
| FunctionDef | `gen_sku_master` | 230 | docstringなし（下のコード参照） |
| FunctionDef | `gen_node_master` | 269 | docstringなし（下のコード参照） |
| FunctionDef | `gen_sc_tree_master` | 306 | docstringなし（下のコード参照） |
| FunctionDef | `gen_lane_assignment` | 331 | docstringなし（下のコード参照） |
| FunctionDef | `gen_route_master` | 343 | docstringなし（下のコード参照） |
| FunctionDef | `gen_edge_cost_master` | 356 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_tariff_rule` | 367 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_transfer_price_rule` | 382 | docstringなし（下のコード参照） |
| FunctionDef | `gen_node_cost_master` | 397 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_edge_cost_rule` | 435 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_node_cost_rule` | 452 | ppc_node_cost_rule.csv -- Factory conversion_cost + DC warehouse_cost |
| FunctionDef | `gen_ppc_node_profit_zone` | 474 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_profit_zone_rule` | 489 | docstringなし（下のコード参照） |
| FunctionDef | `gen_holiday_calendar` | 503 | docstringなし（下のコード参照） |
| FunctionDef | `gen_inventory_master` | 522 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_supplier_cost` | 538 | docstringなし（下のコード参照） |
| FunctionDef | `gen_ppc_market_price` | 558 | docstringなし（下のコード参照） |
| FunctionDef | `gen_demand_forecast` | 570 | docstringなし（下のコード参照） |
| FunctionDef | `gen_capacity_plan` | 587 | docstringなし（下のコード参照） |
| FunctionDef | `gen_fx_rate` | 610 | docstringなし（下のコード参照） |
| FunctionDef | `main` | 618 | docstringなし（下のコード参照） |
