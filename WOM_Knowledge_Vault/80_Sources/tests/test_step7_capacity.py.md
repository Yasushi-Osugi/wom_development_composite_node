...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_step7_capacity.py) · [原文テキスト](../../90_Raw/tests/test_step7_capacity.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `build_tree_with_demand` | 43 | Single-region, single-SKU demo tree. |
| FunctionDef | `test_cap_hard_sealing` | 84 | CapHard=2, demand=4 at MOM week W06 (backward-propagated from leaf W10). |
| FunctionDef | `test_cap_soft_violation_no_movement` | 121 | CapSoft=2 only (CapHard=0), demand=4. |
| FunctionDef | `test_combined_cap_hard_soft` | 158 | CapHard=3, CapSoft=2, demand=5. |
| FunctionDef | `test_capacity_profile_apply` | 193 | Use CapacityProfile + apply_capacity_profile to set CapHard on MOM node. |
| FunctionDef | `test_build_mom_capacity_profile` | 229 | build_mom_capacity_profile creates entries for all MOM nodes. |
| FunctionDef | `test_e2e_cap_hard_causes_leaf_shortfall` | 263 | Full E2E test: Lot_ID identity-matched decouple design. |
| FunctionDef | `test_capacity_load_report` | 346 | After forward planning, build_capacity_load_report returns |
