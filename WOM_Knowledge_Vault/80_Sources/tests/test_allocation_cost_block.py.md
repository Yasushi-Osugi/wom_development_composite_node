...
tags: [wom, code]
---
# tests/test_allocation_cost_block.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_cost_block.py) · [原文テキスト](../../90_Raw/tests/test_allocation_cost_block.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_blocks` | 25 | docstringなし（下のコード参照） |
| FunctionDef | `test_derived_blocks_match_prototype` | 30 | docstringなし（下のコード参照） |
| FunctionDef | `test_transfer_price_usd` | 59 | docstringなし（下のコード参照） |
| FunctionDef | `test_material_base_is_six` | 64 | docstringなし（下のコード参照） |
| FunctionDef | `test_jp_margin_pct_4points` | 77 | docstringなし（下のコード参照） |
| FunctionDef | `test_derivation_conservation_identity` | 85 | §5.1 恒等式：導出ブロック合計 × レート = unit_cost_ex_tariff（保存性）。 |
