...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_ppc_vertical_slice.py) · [原文テキスト](../../90_Raw/tests/test_ppc_vertical_slice.py.txt)

基準: `7d6c7d734ebdcb7b213bab3116ca59d3d4935f55`。[[00_Start/Source_Policy|出典と読み方]]

関連機能: [[10_Functions/01_Data_Building|Data Building]] / [[10_Functions/03_Planning|Planning]] / [[10_Functions/04_Evaluation|Evaluation]]
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `rules` | 66 | Load PPC rule set from data/ppc/. |
| FunctionDef | `fx` | 72 | docstringなし（下のコード参照） |
| FunctionDef | `sc_paths` | 77 | docstringなし（下のコード参照） |
| FunctionDef | `_sales` | 81 | Build a minimal sales_records DataFrame. |
| FunctionDef | `_run_engine` | 91 | docstringなし（下のコード参照） |
| ClassDef | `TestT1_ForwardPropagation` | 106 | docstringなし（下のコード参照） |
| ClassDef | `TestT2_TransferPrice` | 164 | docstringなし（下のコード参照） |
| ClassDef | `TestT3_TariffCrossBorder` | 219 | docstringなし（下のコード参照） |
| ClassDef | `TestT4_LandedCost` | 270 | docstringなし（下のコード参照） |
| ClassDef | `TestT5_FX` | 321 | docstringなし（下のコード参照） |
| ClassDef | `TestT6_BackwardLotBased` | 389 | docstringなし（下のコード参照） |
| ClassDef | `TestT7_Reconciliation` | 437 | docstringなし（下のコード参照） |
| ClassDef | `TestT8_ProfitZone` | 518 | docstringなし（下のコード参照） |
| ClassDef | `TestT9_TariffScenarioChange` | 547 | docstringなし（下のコード参照） |
| ClassDef | `TestT10_ExistingPSITestsUnchanged` | 617 | docstringなし（下のコード参照） |
| FunctionDef | `test_supplier_cost_present` | 108 | Supplier cost event is generated for the lot. |
| FunctionDef | `test_supplier_cost_amount` | 119 | Supplier cost matches ppc_supplier_cost.csv value × FX rate. |
| FunctionDef | `test_conversion_cost_accumulated` | 135 | Conversion cost (MOM) is also present and non-zero. |
| FunctionDef | `test_supplier_plus_conversion_in_accumulator` | 150 | Accumulator supplier_cost_base + conversion_cost_base match events. |
| FunctionDef | `test_transfer_price_formula` | 166 | transfer_price = MOM_accumulated_cost × (1 + 0.10). |
| FunctionDef | `test_transfer_price_event_fired_once` | 187 | Exactly one transfer_price_set event per lot. |
| FunctionDef | `test_transfer_price_same_for_same_week_and_product` | 201 | Two lots in same week, same product, same MOM → same transfer_price. |
| FunctionDef | `test_tariff_on_cn_jp_edge` | 221 | tariff_cost event exists for MOM_China→DAD_Japan (CN→JP). |
| FunctionDef | `test_no_tariff_on_domestic_jp_edge` | 239 | No tariff event for DAD_Japan→JP_Channel (domestic JP). |
| FunctionDef | `test_tariff_on_jp_us_edge` | 250 | tariff_cost event exists for DAD_Japan→US_Channel (JP→US). |
| FunctionDef | `test_landed_cost_components` | 272 | landed_cost = transfer_price + logistics(CN→JP) + insurance + tariff(CN→JP) |
| FunctionDef | `test_landed_cost_per_unit` | 303 | Concrete sanity: landed cost in JPY must be less than JP market price. |
| FunctionDef | `test_all_events_have_fx_fields` | 323 | Every PPCEvent (except base-currency ones) has non-zero fx_rate. |
| FunctionDef | `test_us_revenue_converted_correctly` | 335 | US revenue: 1024 USD × 150.0 (W01 rate) = 153,600 JPY. |
| FunctionDef | `test_jp_revenue_is_native_jpy` | 350 | JP revenue is already JPY → fx_rate = 1.0. |
| FunctionDef | `test_fx_fallback_for_missing_week` | 361 | FX fallback: if week has no rate, latest prior week is used and |
| FunctionDef | `test_kpi_revenue_in_base_currency` | 373 | KPI total_revenue_base is sum of per-lot revenues in JPY. |
| FunctionDef | `test_jp_us_different_allowable_costs` | 391 | Two lots from same MOM in same week, sold at JP vs US channels, |
| FunctionDef | `test_backward_allowable_positive_for_jp` | 412 | JP lot backward allowable cost should be positive (market covers MOM costs). |
| FunctionDef | `test_backward_event_per_lot` | 419 | One backward_allowable event generated per lot. |
| FunctionDef | `_make_loss_sales` | 439 | A lot sold at an artificially LOW price (simulated by custom rules override). |
| FunctionDef | `test_no_trust_events_normal_scenario` | 452 | Base scenario (JP+US W01) should have no NEGATIVE_MARGIN trust events. |
| FunctionDef | `test_trust_event_fires_for_loss_lot` | 465 | When tariff rate is raised to 200% (via modified rules), forward cost |
| FunctionDef | `test_lot_reconciliation_df_has_all_lots` | 502 | lot_reconciliation DataFrame has one row per lot. |
| FunctionDef | `test_profit_zones_all_present` | 520 | Profit zone summary contains expected zones. |
| FunctionDef | `test_channel_zones_have_revenue` | 532 | OUTBOUND_CHANNEL_PROFIT zone has positive revenue. |
| FunctionDef | `_run_with_tariff` | 549 | docstringなし（下のコード参照） |
| FunctionDef | `test_higher_tariff_increases_cost` | 571 | Doubling CN→JP tariff rate increases total forward cost. |
| FunctionDef | `test_higher_tariff_reduces_gross_profit` | 580 | Higher tariff reduces gross profit (revenue unchanged). |
| FunctionDef | `test_landed_cost_changes_with_tariff` | 589 | landed_cost_total event amount changes when tariff rate changes. |
| FunctionDef | `test_zero_tariff_means_no_tariff_events` | 602 | When tariff rate = 0.0, tariff_cost events have amount_base = 0. |
| FunctionDef | `test_push_pull_module_importable` | 619 | wom.engine.push_pull imports without error. |
| FunctionDef | `test_plan_node_importable` | 624 | wom.model.plan_node imports without error. |
| FunctionDef | `test_ppc_does_not_modify_plan_node` | 629 | PlanNode class has no PPC-related attributes added by ppc package. |
| FunctionDef | `test_ppc_package_does_not_import_gui` | 640 | wom.ppc package does not import tkinter or wom.gui. |
| FunctionDef | `get_landed` | 594 | docstringなし（下のコード参照） |
