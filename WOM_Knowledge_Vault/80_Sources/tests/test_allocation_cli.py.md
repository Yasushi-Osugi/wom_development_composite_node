...
tags: [wom, code]
---
# tests/test_allocation_cli.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_cli.py) · [原文テキスト](../../90_Raw/tests/test_allocation_cli.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_read` | 23 | docstringなし（下のコード参照） |
| FunctionDef | `test_run_writes_seven_files` | 28 | docstringなし（下のコード参照） |
| FunctionDef | `test_profit_surface_row_count` | 37 | docstringなし（下のコード参照） |
| FunctionDef | `test_plateau_max_profit_regression` | 47 | docstringなし（下のコード参照） |
| FunctionDef | `test_switching_points_in_output` | 60 | docstringなし（下のコード参照） |
| FunctionDef | `test_cost_block_derived_output` | 68 | docstringなし（下のコード参照） |
| FunctionDef | `test_scenario_loader_time_series_flag` | 78 | docstringなし（下のコード参照） |
