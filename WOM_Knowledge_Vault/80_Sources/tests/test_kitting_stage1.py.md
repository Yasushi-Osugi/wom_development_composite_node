...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_kitting_stage1.py) · [原文テキスト](../../90_Raw/tests/test_kitting_stage1.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_build_tree` | 51 | One MOM root ("mom") with children child0, child1, ... whose |
| FunctionDef | `_new_fp` | 85 | ForwardPlanner with the per-run state that run() normally sets up |
| FunctionDef | `_propagate` | 94 | Simulate child's actual (physically matched) shipment of `lot_ids` |
| FunctionDef | `test_kitting_attribute_exists_and_defaults_empty` | 106 | docstringなし（下のコード参照） |
| FunctionDef | `test_kitting_required_excludes_confluence` | 116 | docstringなし（下のコード参照） |
| FunctionDef | `test_kitting_required_single_assembly_child` | 123 | docstringなし（下のコード参照） |
| FunctionDef | `test_kitting_status_missing_and_is_complete` | 128 | docstringなし（下のコード参照） |
| FunctionDef | `test_assembly_siblings_recorded_under_shared_assembly_week_even_when_arrival_weeks_differ` | 155 | The core scenario this design exists for: Battery arrives late, |
| FunctionDef | `test_partial_delivery_missing_reports_which_child_is_late` | 190 | docstringなし（下のコード参照） |
| FunctionDef | `test_confluence_child_never_writes_kitting` | 209 | docstringなし（下のコード参照） |
| FunctionDef | `test_confluence_only_parent_has_empty_kitting` | 228 | docstringなし（下のコード参照） |
| FunctionDef | `test_kitting_fallback_when_lot_not_in_parent_demand` | 243 | A lot_id that never appears in parent.psi4demand[*][S] (e.g. it did |
| FunctionDef | `test_gate_flag_is_false_and_incomplete_kit_still_enters_p` | 269 | docstringなし（下のコード参照） |
