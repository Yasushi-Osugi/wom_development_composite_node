...
tags: [wom, code]
---
# tests/test_allocation_regime_map.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_regime_map.py) · [原文テキスト](../../90_Raw/tests/test_allocation_regime_map.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_nonempty` | 34 | docstringなし（下のコード参照） |
| FunctionDef | `soysauce_blocks` | 39 | docstringなし（下のコード参照） |
| FunctionDef | `test_regime_map_reproduces_switching_points` | 47 | material_usd=6.0 の水平断面が switching_points() の切替点（117円/119円）を |
| FunctionDef | `test_regime_map_axis_tariff` | 72 | tariff_rate:US を軸に取れること |
| FunctionDef | `test_regime_map_regime_ids_consistent` | 87 | regime_ids と regime_labels の対応が全点で整合すること |
| FunctionDef | `test_regime_map_negative_margin_mask` | 99 | 高FX・高原料の隅で JP が negative_margin_mask に入ること |
| FunctionDef | `test_regime_map_invalid_axis` | 114 | 不正な軸名は ValueError |
| FunctionDef | `test_plot_allocation_merit_order` | 129 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_allocation_merit_shift` | 140 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_regime_map` | 151 | docstringなし（下のコード参照） |
| FunctionDef | `test_cli_demo_generates_all` | 162 | docstringなし（下のコード参照） |
