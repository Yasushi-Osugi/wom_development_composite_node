...
tags: [wom, code]
---
# tests/test_operating_calendar.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_operating_calendar.py) · [原文テキスト](../../90_Raw/tests/test_operating_calendar.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_demo` | 34 | docstringなし（下のコード参照） |
| FunctionDef | `test_default_no_calendar_is_always_open` | 49 | docstringなし（下のコード参照） |
| FunctionDef | `test_set_operating_shifts_open_closed` | 64 | docstringなし（下のコード参照） |
| FunctionDef | `test_offset_week_skips_zero_shift_weeks` | 82 | docstringなし（下のコード参照） |
| FunctionDef | `test_loader_reads_operating_calendar` | 97 | docstringなし（下のコード参照） |
