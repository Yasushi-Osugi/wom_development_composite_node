...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_pareto_front.py) · [原文テキスト](../../90_Raw/tests/test_pareto_front.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| ClassDef | `TestObjectiveFunctions` | 16 | 目的関数テスト（2個） |
| ClassDef | `TestParetoFront` | 52 | Pareto Front テスト（3個） |
| ClassDef | `TestTradeoffRatios` | 113 | トレードオフテスト（2個） |
| ClassDef | `TestEndToEnd` | 153 | 統合テスト（1個） |
| FunctionDef | `sample_suppliers` | 185 | V0 (Phase 3): plan粒度のテスト用サンプルサプライヤー |
| ClassDef | `TestPlanGranularity` | 210 | V0: 1配分案=1解の粒度（Phase 3-0） |
| FunctionDef | `test_compute_allocation_objectives_simple` | 19 | 単一サプライヤー配分 |
| FunctionDef | `test_compute_allocation_objectives_mixed` | 35 | 複数サプライヤー混合配分（異なる規模・単価の配分案） |
| FunctionDef | `test_pareto_front_single_solution` | 55 | 配分案が 1 つのみ |
| FunctionDef | `test_pareto_front_all_on_front` | 72 | 全案が Pareto Front 上（互いにトレードオフの関係） |
| FunctionDef | `test_pareto_front_dominated_solutions` | 95 | 支配関係がある配分（1件は他に完全に劣後） |
| FunctionDef | `test_tradeoff_ratios_cost_vs_quality` | 116 | docstringなし（下のコード参照） |
| FunctionDef | `test_tradeoff_ratios_cost_vs_lead_time` | 134 | docstringなし（下のコード参照） |
| FunctionDef | `test_pareto_export_to_json` | 156 | docstringなし（下のコード参照） |
| FunctionDef | `test_plan_objectives_matches_merit_order_totals` | 213 | compute_plan_objectives() が merit_order result の |
| FunctionDef | `test_pareto_front_from_plans_multiple_on_front` | 226 | 異なる制約で生成した複数の配分案を与えたとき、 |
| FunctionDef | `test_pareto_front_record_granularity_backward_compat` | 254 | 従来の __init__(allocations) 経路が補正前と同じ結果を返すこと |
| FunctionDef | `test_lead_time_metric_max_option` | 269 | lead_time_metric='max' で最大リードタイムが採られること |
