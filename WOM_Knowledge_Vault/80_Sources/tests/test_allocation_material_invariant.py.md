...
tags: [wom, code]
---
# tests/test_allocation_material_invariant.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_material_invariant.py) · [原文テキスト](../../90_Raw/tests/test_allocation_material_invariant.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_model_dirs_with_ga_scenario_master` | 54 | docstringなし（下のコード参照） |
| FunctionDef | `_base_material_price_usd` | 60 | docstringなし（下のコード参照） |
| FunctionDef | `test_at_least_soysauce_and_oil_present` | 71 | 掃き出し範囲の固定（letter §3.6）: サンプルケースが増減したら気づけるように。 |
| FunctionDef | `test_base_material_price_matches_derived_cost_block` | 79 | base シナリオ（行0）の material_price_usd == derive_cost_blocks() の material_usd_base。 |
| FunctionDef | `test_oil_kl100kbbl_is_a_known_unverified_mismatch` | 103 | §5.1: KL100KBBL を無言で通さない。 |
