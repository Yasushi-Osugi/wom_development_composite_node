...
tags: [wom, code]
---
# tests/test_init_stock_days_wiring.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_init_stock_days_wiring.py) · [原文テキスト](../../90_Raw/tests/test_init_stock_days_wiring.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_rows` | 37 | 最小の有効ツリー（InBound: leaf_in->mom, OutBound: supply_point->dad->leaf_out）。 |
| FunctionDef | `_find` | 67 | docstringなし（下のコード参照） |
| FunctionDef | `_dc_node` | 77 | OutBound の supply_point root 配下から DC_X を取得。 |
| FunctionDef | `test_init_stock_wks_ceil_semantics` | 89 | docstringなし（下のコード参照） |
| FunctionDef | `test_init_stock_days_default_is_zero_on_plannode` | 100 | docstringなし（下のコード参照） |
| FunctionDef | `test_csv_init_stock_days_reaches_node` | 111 | docstringなし（下のコード参照） |
| FunctionDef | `test_csv_init_stock_days_various_values_reach_node` | 119 | docstringなし（下のコード参照） |
| FunctionDef | `test_missing_column_defaults_to_zero` | 132 | docstringなし（下のコード参照） |
