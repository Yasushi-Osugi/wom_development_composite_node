...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_ppc_multi_supplier.py) · [原文テキスト](../../90_Raw/tests/test_ppc_multi_supplier.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `rules` | 55 | docstringなし（下のコード参照） |
| FunctionDef | `fx` | 106 | docstringなし（下のコード参照） |
| FunctionDef | `_acc` | 110 | docstringなし（下のコード参照） |
| ClassDef | `TestResolveNodeList` | 119 | docstringなし（下のコード参照） |
| ClassDef | `TestMultiSupplierForwardPropagation` | 141 | docstringなし（下のコード参照） |
| ClassDef | `TestNodePLSummary` | 195 | docstringなし（下のコード参照） |
| FunctionDef | `test_bare_string_wraps_to_single_item_list` | 121 | docstringなし（下のコード参照） |
| FunctionDef | `test_dict_of_strings_legacy_single_supplier` | 124 | docstringなし（下のコード参照） |
| FunctionDef | `test_bare_list_multi_supplier_same_for_all_products` | 128 | docstringなし（下のコード参照） |
| FunctionDef | `test_dict_of_lists_multi_supplier_per_product` | 132 | docstringなし（下のコード参照） |
| FunctionDef | `test_one_supplier_cost_event_per_tier1_supplier` | 143 | docstringなし（下のコード参照） |
| FunctionDef | `test_supplier_cost_base_sums_all_suppliers` | 154 | docstringなし（下のコード参照） |
| FunctionDef | `test_dict_of_lists_multi_product_form` | 165 | GENERIC scenario shape: dict[product_id -> list[node_id]]. |
| FunctionDef | `test_legacy_single_supplier_string_unaffected` | 178 | Cookie/iPhone/Rice-style single-supplier call still works as before. |
| FunctionDef | `test_each_supplier_gets_its_own_pl_row` | 197 | docstringなし（下のコード参照） |
| FunctionDef | `test_empty_events_returns_empty_frame_with_expected_columns` | 221 | docstringなし（下のコード参照） |
