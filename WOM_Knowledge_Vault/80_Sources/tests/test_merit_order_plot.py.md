...
tags: [wom, code]
---
# tests/test_merit_order_plot.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_merit_order_plot.py) · [原文テキスト](../../90_Raw/tests/test_merit_order_plot.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_nonempty` | 36 | docstringなし（下のコード参照） |
| FunctionDef | `demo_analyzer` | 41 | docstringなし（下のコード参照） |
| FunctionDef | `demo_result` | 48 | docstringなし（下のコード参照） |
| FunctionDef | `demo_horizon` | 53 | docstringなし（下のコード参照） |
| FunctionDef | `demo_pareto` | 66 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_merit_order_curve` | 82 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_merit_order_curve_unmet_demand` | 87 | required_qty が総供給量を超えるケースで λ 注記なしでも例外にならないこと |
| FunctionDef | `test_plot_merit_order_shift` | 100 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_regime_matrix` | 115 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_regime_timeline` | 120 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_pareto_scatter` | 125 | docstringなし（下のコード参照） |
| FunctionDef | `test_plot_parallel_coordinates` | 131 | docstringなし（下のコード参照） |
| FunctionDef | `test_build_supplier_share_axes_union_and_order` | 143 | 登場サプライヤーの和集合から、Merit Order順（単価昇順）の軸が作られ、 |
| FunctionDef | `test_build_supplier_share_axes_others_bucket` | 178 | top_k を超えるサプライヤーが Others に集約され、各案の share 合計が1.0であること |
| FunctionDef | `test_weeks_label_singular_plural` | 191 | F3: 週数の単複表記 |
