...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_step8_push_pull.py) · [原文テキスト](../../90_Raw/tests/test_step8_push_pull.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `build_base` | 23 | docstringなし（下のコード参照） |
| FunctionDef | `get_node` | 40 | docstringなし（下のコード参照） |
| FunctionDef | `test_pull_baseline` | 48 | docstringなし（下のコード参照） |
| FunctionDef | `test_push_fixed_overwrite` | 61 | docstringなし（下のコード参照） |
| FunctionDef | `test_push_vs_pull_leaf_in_p` | 80 | docstringなし（下のコード参照） |
| FunctionDef | `test_push_replenishment` | 100 | docstringなし（下のコード参照） |
| FunctionDef | `test_push_region_distribution` | 115 | docstringなし（下のコード参照） |
| FunctionDef | `test_mark_pull_mode_reset` | 135 | docstringなし（下のコード参照） |
| FunctionDef | `test_get_push_pull_summary` | 154 | docstringなし（下のコード参照） |
| FunctionDef | `test_e2e_push_produces_buffer_inventory` | 180 | PUSH=6, demand=4: leaf fulfills demand; MOM holds surplus as inventory. |
| FunctionDef | `test_dad_inventory_surplus_opening_inv` | 228 | v1r0m2 DAD 回転在庫テスト (surplus case): |
| FunctionDef | `test_dad_inventory_cap_hard_shortfall` | 254 | Lot_ID identity-matched decouple 設計の確認テスト: |
