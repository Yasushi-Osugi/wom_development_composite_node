...
tags: [wom, code]
---
# tests/test_capacity_soft.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_capacity_soft.py) · [原文テキスト](../../90_Raw/tests/test_capacity_soft.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_demo_tree` | 46 | single-SKU / single-region の demo tree（W01..W26）。MOM = get_in_root。 |
| FunctionDef | `test_loader_reads_cap_soft_column_sku_aggregate` | 64 | node_name 列なしの capacity_plan（sku 集約→MOM）で cap_soft 列を読む。 |
| FunctionDef | `test_loader_reads_cap_soft_column_node_name` | 81 | node_name 列ありの capacity_plan でも cap_soft が対象ノードに乗る事。 |
| FunctionDef | `test_loader_absent_cap_soft_column_is_backward_compatible` | 96 | cap_soft 列が **無い** 従来 CSV：cap_hard は設定され、cap_soft は既定 0.0 のまま |
| FunctionDef | `test_cap_soft_from_csv_drives_forward_violation_no_movement` | 119 | §11.1 の Unit 例：cap_soft=2 / cap_hard=4 / 需要=6。 |
