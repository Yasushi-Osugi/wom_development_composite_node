...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_stage3a2_kitting_gate.py) · [原文テキスト](../../90_Raw/tests/test_stage3a2_kitting_gate.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `test_gate_flag_is_enabled` | 41 | docstringなし（下のコード参照） |
| FunctionDef | `_mk` | 50 | docstringなし（下のコード参照） |
| FunctionDef | `_build_tree` | 56 | docstringなし（下のコード参照） |
| FunctionDef | `_run_pipeline` | 76 | cap_hard_by_leaf: [(PlanNode, cap), ...] -- a list, not a dict, since |
| FunctionDef | `_seed_demand` | 92 | docstringなし（下のコード参照） |
| FunctionDef | `test_complete_kit_enters_p_exactly_once` | 98 | docstringなし（下のコード参照） |
| FunctionDef | `test_partial_kit_stays_in_every_yards_inventory` | 114 | Cap one component (ECU) far below demand -- lots missing their ECU |
| FunctionDef | `test_mass_conservation_total_lot_count` | 147 | Every demand lot ends up EITHER paid out (in MOM.P, exactly once) OR |
| FunctionDef | `test_bom_qty_does_not_affect_gate_judgment` | 179 | bom_qty is a physical-quantity display multiplier; the gate must |
| FunctionDef | `test_kitting_missing_still_reports_correctly_under_gate` | 192 | docstringなし（下のコード参照） |
| FunctionDef | `test_no_stockyard_children_unaffected` | 210 | A plain assembly node with ordinary leaf_in children (no stockyard) |
| FunctionDef | `test_bom_test_2026_vehicle_assy_p_matches_s` | 245 | docstringなし（下のコード参照） |
| FunctionDef | `test_ev_europe_2026_import_factory_p_matches_real_demand` | 257 | docstringなし（下のコード参照） |
