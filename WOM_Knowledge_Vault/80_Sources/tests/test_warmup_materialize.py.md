...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_warmup_materialize.py) · [原文テキスト](../../90_Raw/tests/test_warmup_materialize.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_write` | 29 | docstringなし（下のコード参照） |
| FunctionDef | `_make_model` | 34 | 実データ 2027-W01..W03（JP/US 需要、Bottling/Materials 能力、Bottling shift）。 |
| FunctionDef | `_read` | 54 | docstringなし（下のコード参照） |
| FunctionDef | `_rows` | 59 | docstringなし（下のコード参照） |
| FunctionDef | `test_week_minus_and_weeks_between_cross_year` | 66 | docstringなし（下のコード参照） |
| FunctionDef | `test_resolve_effective_start` | 75 | docstringなし（下のコード参照） |
| FunctionDef | `test_first_nonzero_demand_week` | 83 | docstringなし（下のコード参照） |
| FunctionDef | `test_materialize_demand_zero_and_copy` | 92 | docstringなし（下のコード参照） |
| FunctionDef | `test_idempotent_and_byte_stable` | 124 | docstringなし（下のコード参照） |
| FunctionDef | `test_write_if_needed_dry_run_does_not_write` | 137 | docstringなし（下のコード参照） |
| FunctionDef | `test_no_config_is_noop` | 147 | docstringなし（下のコード参照） |
| FunctionDef | `test_warmup_lt_zero_strips_existing_warmup` | 158 | docstringなし（下のコード参照） |
