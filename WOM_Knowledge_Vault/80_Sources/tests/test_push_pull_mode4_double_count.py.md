...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_push_pull_mode4_double_count.py) · [原文テキスト](../../90_Raw/tests/test_push_pull_mode4_double_count.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_push_config_content` | 75 | docstringなし（下のコード参照） |
| FunctionDef | `_ops_for_lt` | 83 | docstringなし（下のコード参照） |
| FunctionDef | `_run_case` | 93 | Apply `ops` to the real apparel-us-2026 folder under guard, run the |
| FunctionDef | `test_mode4_p_sum_matches_base` | 109 | docstringなし（下のコード参照） |
| FunctionDef | `test_mode4_no_downstream_carry_over` | 123 | docstringなし（下のコード参照） |
| FunctionDef | `test_mode4_lt_below_tau_shows_genuine_shortage` | 136 | push_lead_time_weeks=2 < tau=3 (Fabric_CN's own transit lt_wks) is a |
| FunctionDef | `test_mode4_no_stale_weeks_outside_write_set` | 161 | Every week of Fabric_CN.psi4supply[P] must be explainable as either |
| FunctionDef | `test_mode4_only_flag_still_skips_p_schedule` | 193 | mode_only=True must still bypass Mode4's P-schedule entirely (and |
