...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_regime_map.py) · [原文テキスト](../../90_Raw/tests/test_regime_map.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `make_merit_order_result` | 39 | docstringなし（下のコード参照） |
| ClassDef | `TestClassificationFunctions` | 47 | 分類関数（モジュールレベル）のテスト（6個） |
| ClassDef | `TestRegimeMapping` | 69 | マッピングテスト（3個） |
| ClassDef | `TestIntegration` | 126 | 統合テスト（3個） |
| FunctionDef | `test_classify_demand_level_low` | 50 | docstringなし（下のコード参照） |
| FunctionDef | `test_classify_demand_level_medium` | 53 | docstringなし（下のコード参照） |
| FunctionDef | `test_classify_demand_level_high` | 56 | docstringなし（下のコード参照） |
| FunctionDef | `test_classify_supply_tightness_tight` | 59 | docstringなし（下のコード参照） |
| FunctionDef | `test_classify_supply_tightness_balanced` | 62 | docstringなし（下のコード参照） |
| FunctionDef | `test_classify_supply_tightness_surplus` | 65 | docstringなし（下のコード参照） |
| FunctionDef | `test_regime_cell_mapping` | 72 | 9 セル全て正しくマッピングされることを確認 |
| FunctionDef | `test_recommended_strategy_lookup` | 89 | 全 9 戦略が正しく取得されることを確認 |
| FunctionDef | `test_strategy_actions_consistency` | 103 | 推奨戦略とアクション内容に矛盾がないことを確認 |
| FunctionDef | `test_classify_single_week` | 129 | Merit Order 結果から regime 出力まで一連処理 |
| FunctionDef | `test_classify_horizon_12weeks` | 147 | 12 週のホライズン分析 |
| FunctionDef | `test_export_to_json_regime` | 185 | JSON エクスポート・フォーマット確認 |
