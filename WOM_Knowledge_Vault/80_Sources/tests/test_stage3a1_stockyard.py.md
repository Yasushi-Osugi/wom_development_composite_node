...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_stage3a1_stockyard.py) · [原文テキスト](../../90_Raw/tests/test_stage3a1_stockyard.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `test_node_type_stockyard_constant_exists` | 64 | docstringなし（下のコード参照） |
| FunctionDef | `test_csv_lt_wks_zero_is_honoured_not_forced_to_one` | 72 | Regression for the bug this stage found: a non-root node with |
| FunctionDef | `test_csv_lt_wks_missing_column_still_defaults_to_1` | 93 | Unaffected-case regression: a row with NO lt_wks column at all must |
| FunctionDef | `_mk` | 112 | docstringなし（下のコード参照） |
| FunctionDef | `_build_without_yard` | 118 | docstringなし（下のコード参照） |
| FunctionDef | `_build_with_yard` | 132 | docstringなし（下のコード参照） |
| FunctionDef | `_run_pipeline` | 150 | docstringなし（下のコード参照） |
| FunctionDef | `_summary` | 161 | docstringなし（下のコード参照） |
| FunctionDef | `_kitting_complete_total` | 167 | docstringなし（下のコード参照） |
| FunctionDef | `test_yard_insertion_now_deduplicates_mom_p_via_gate` | 177 | Historical note: at Stage 3a-1 (record-only, no gate keeping), this |
| FunctionDef | `test_yard_insertion_leaves_leaf_in_demand_and_supply_unchanged` | 226 | docstringなし（下のコード参照） |
| FunctionDef | `test_kitting_required_reflects_yard_names_after_insertion` | 250 | The direct children of the assembly node are now the Yards, so |
| FunctionDef | `test_yard_own_kitting_is_trivial_single_child` | 260 | docstringなし（下のコード参照） |
| FunctionDef | `test_boundary_week_lot_not_silently_dropped_through_yard` | 271 | Direct regression for the bug this stage found: a lot whose demand |
| FunctionDef | `test_ev_europe_2026_import_chain_matches_post_gate_baseline` | 308 | Historical note: at Stage 3a-1 (record-only), this asserted P=23,835 / |
| FunctionDef | `test_ev_europe_2026_local_chain_matches_pre_yard_baseline` | 333 | Local chain has no push_config -- pure PULL. Same missing count |
| FunctionDef | `test_bom_test_2026_three_cases_gate_deduplicates_vehicle_assy_p` | 343 | Historical note: at Stage 3a-1 (record-only), Vehicle_Assy_P was |
| FunctionDef | `total` | 162 | docstringなし（下のコード参照） |
| FunctionDef | `find` | 232 | docstringなし（下のコード参照） |
| FunctionDef | `find_leaf` | 282 | docstringなし（下のコード参照） |
| FunctionDef | `total` | 319 | docstringなし（下のコード参照） |
| FunctionDef | `find` | 377 | docstringなし（下のコード参照） |
