...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_step10_hooks.py) · [原文テキスト](../../90_Raw/tests/test_step10_hooks.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_make_bus` | 19 | docstringなし（下のコード参照） |
| ClassDef | `TestHookBus` | 25 | docstringなし（下のコード参照） |
| ClassDef | `TestWOMPlugin` | 85 | docstringなし（下のコード参照） |
| ClassDef | `TestDemandSmoothingPlugin` | 105 | docstringなし（下のコード参照） |
| ClassDef | `TestCapacityOverridePlugin` | 179 | docstringなし（下のコード参照） |
| FunctionDef | `test_fire_calls_listener` | 26 | docstringなし（下のコード参照） |
| FunctionDef | `test_multiple_listeners_same_hook` | 34 | docstringなし（下のコード参照） |
| FunctionDef | `test_unregister` | 42 | docstringなし（下のコード参照） |
| FunctionDef | `test_clear` | 51 | docstringなし（下のコード参照） |
| FunctionDef | `test_invalid_hook_raises` | 61 | docstringなし（下のコード参照） |
| FunctionDef | `test_listener_exception_is_non_fatal` | 66 | A crashing listener should not prevent other listeners from running. |
| FunctionDef | `test_listener_count` | 75 | docstringなし（下のコード参照） |
| FunctionDef | `test_register_adds_listeners` | 86 | docstringなし（下のコード参照） |
| FunctionDef | `test_default_callbacks_are_noop` | 93 | Calling default hooks must not raise. |
| FunctionDef | `_make_sc_tree_with_demand` | 106 | Build a minimal sc_tree mock where get_in_root() returns a node |
| FunctionDef | `test_smoothing_reduces_spike` | 140 | docstringなし（下のコード参照） |
| FunctionDef | `test_smoothing_preserves_total` | 158 | docstringなし（下のコード参照） |
| FunctionDef | `test_no_file_no_crash` | 180 | docstringなし（下のコード参照） |
| FunctionDef | `test_applies_override` | 190 | docstringなし（下のコード参照） |
| ClassDef | `FakeSCTree` | 133 | docstringなし（下のコード参照） |
| ClassDef | `FakeSCTree` | 214 | docstringなし（下のコード参照） |
| FunctionDef | `get_in_root` | 135 | docstringなし（下のコード参照） |
| FunctionDef | `get_in_root` | 216 | docstringなし（下のコード参照） |
