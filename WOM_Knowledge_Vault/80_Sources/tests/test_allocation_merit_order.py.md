...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_merit_order.py) · [原文テキスト](../../90_Raw/tests/test_allocation_merit_order.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `soysauce_blocks` | 23 | docstringなし（下のコード参照） |
| FunctionDef | `test_merit_order_descending` | 28 | ブロックが単位マージン降順（昇順でない）であること |
| FunctionDef | `test_merit_order_lambda_is_marginal_market_margin` | 40 | λ が限界市場のマージンと一致すること |
| FunctionDef | `test_merit_order_soysauce_regression` | 52 | soysauce 実データで λ=750, x=(0.1544, 0.4228, 0.4228), 利益 135,529,822.5 JPY |
| FunctionDef | `test_merit_order_capacity_surplus` | 68 | 需要合計 < 能力のとき λ=0、marginal_market が None、idle > 0 |
| FunctionDef | `test_merit_order_excludes_negative_margin` | 95 | FX200/$8 で JP（-105）が excluded に入り、配分されないこと |
| FunctionDef | `test_compare_with_grid_soysauce` | 107 | グリッド最適 132,133,072.5 JPY、乖離 3,396,750 JPY（+2.57%） |
| FunctionDef | `test_gap_attributable_to_grid_resolution` | 122 | soysauce では乖離が grid_idle × lambda で厳密に説明でき、構造由来の残差はゼロ |
| FunctionDef | `test_expected_gap_equals_idle_times_lambda` | 139 | 乖離 = 格子最適点の遊休能力 × λ という恒等式（§3.5 rev.2 の判定の根拠） |
| FunctionDef | `test_merit_order_x_sums_to_one_when_capacity_binding` | 156 | 能力が制約になるとき Σx = 1.0 |
