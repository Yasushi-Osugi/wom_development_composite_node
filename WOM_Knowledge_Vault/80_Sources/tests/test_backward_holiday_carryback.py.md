...
tags: [wom, code]
---
# tests/test_backward_holiday_carryback.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_backward_holiday_carryback.py) · [原文テキスト](../../90_Raw/tests/test_backward_holiday_carryback.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_demo_mom` | 30 | docstringなし（下のコード参照） |
| FunctionDef | `test_closed_week_demand_carried_back_respecting_cap` | 45 | docstringなし（下のコード参照） |
| FunctionDef | `test_carryback_never_exceeds_cap_hard` | 74 | docstringなし（下のコード参照） |
| FunctionDef | `test_no_calendar_backward_compatible` | 100 | docstringなし（下のコード参照） |
