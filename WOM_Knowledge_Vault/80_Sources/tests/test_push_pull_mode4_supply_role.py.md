...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_push_pull_mode4_supply_role.py) · [原文テキスト](../../90_Raw/tests/test_push_pull_mode4_supply_role.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_mk_node` | 65 | docstringなし（下のコード参照） |
| FunctionDef | `_build_flat_tree` | 73 | One MOM decoupling root ("MOM") with flat children, one per |
| FunctionDef | `_build_mixed_tree` | 96 | Final_Assy (MOM, decoupling) |
| FunctionDef | `_seed_membership` | 136 | Directly seed node.psi4demand[w][S] -- simulates what BackwardPlanner |
| FunctionDef | `_p_map` | 146 | docstringなし（下のコード参照） |
| FunctionDef | `test_u1_assembly_children_each_receive_full_recipient_set` | 155 | Battery/Motor/ECU-style assembly siblings: BackwardPlanner already |
| FunctionDef | `test_u2_confluence_children_keep_backward_assigned_membership` | 191 | Confluence siblings: Backward assigned a NON-contiguous split |
| FunctionDef | `test_u3_mixed_multilevel_uses_final_leaf_membership_only` | 218 | docstringなし（下のコード参照） |
| FunctionDef | `test_u4_membership_lookup_is_horizon_wide_not_same_week_only` | 243 | The Letter's own NG example: `lot_id in leaf.psi4demand[future_w][S]` |
| FunctionDef | `test_u5_no_new_lot_id_is_minted` | 274 | docstringなし（下のコード参照） |
| FunctionDef | `test_u7_future_week_beyond_horizon_produces_nothing` | 301 | docstringなし（下のコード参照） |
| FunctionDef | `test_u8_no_duplicate_lot_within_leaf_week` | 320 | docstringなし（下のコード参照） |
| FunctionDef | `test_mode1_fixed_qty_unaffected_by_this_change` | 342 | docstringなし（下のコード参照） |
