...
tags: [wom, code]
---
# tests/test_allocation_nmarket.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_nmarket.py) · [原文テキスト](../../90_Raw/tests/test_allocation_nmarket.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_twin_split` | 36 | market を、経済条件が同一・需要を折半した双子 (name_a, name_b) に割る。 |
| FunctionDef | `test_markets_of_after_twin_split` | 58 | docstringなし（下のコード参照） |
| FunctionDef | `test_four_market_twin_split_invariance` | 63 | 双子分割しても貪欲法の解と真の連続最適は1円も変わらないこと（最重要）。 |
| FunctionDef | `test_four_market_grid_degrades_with_dimension` | 88 | 格子だけは次元の増加で劣化すること（Phase 6-3 が必要な理由を金額で固定）。 |
| FunctionDef | `test_four_market_pipeline_runs` | 105 | A系統の各モジュールが N=4 で動き、4市場すべてを扱うこと。 |
| FunctionDef | `_pos` | 128 | docstringなし（下のコード参照） |
