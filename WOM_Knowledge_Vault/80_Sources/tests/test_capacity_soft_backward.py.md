...
tags: [wom, code]
---
# tests/test_capacity_soft_backward.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_capacity_soft_backward.py) · [原文テキスト](../../90_Raw/tests/test_capacity_soft_backward.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_run_backward` | 32 | demo tree に cap を直接設定し、mom_constrained=True で Backward を走らせる。 |
| FunctionDef | `_totals` | 49 | docstringなし（下のコード参照） |
| FunctionDef | `test_backward_cap_soft_envelope_flagged_within_hard` | 60 | docstringなし（下のコード参照） |
| FunctionDef | `test_backward_cap_soft_envelope_with_hard_overflow` | 78 | docstringなし（下のコード参照） |
| FunctionDef | `test_backward_cap_soft_does_not_move_lots` | 91 | docstringなし（下のコード参照） |
| FunctionDef | `test_backward_cap_soft_equal_hard_is_noop` | 110 | docstringなし（下のコード参照） |
