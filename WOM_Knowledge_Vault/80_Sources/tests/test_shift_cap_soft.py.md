...
tags: [wom, code]
---
# tests/test_shift_cap_soft.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_shift_cap_soft.py) · [原文テキスト](../../90_Raw/tests/test_shift_cap_soft.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_demo` | 31 | docstringなし（下のコード参照） |
| FunctionDef | `test_shifts_derive_cap_soft` | 46 | docstringなし（下のコード参照） |
| FunctionDef | `test_zero_shift_leaves_cap_soft_and_is_closed` | 70 | docstringなし（下のコード参照） |
| FunctionDef | `test_no_cap_hard_no_derivation` | 88 | docstringなし（下のコード参照） |
