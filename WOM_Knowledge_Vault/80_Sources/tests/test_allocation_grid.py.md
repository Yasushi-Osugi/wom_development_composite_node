...
tags: [wom, code]
---
# tests/test_allocation_grid.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_grid.py) · [原文テキスト](../../90_Raw/tests/test_allocation_grid.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `test_simplex_grid_231_points` | 30 | docstringなし（下のコード参照） |
| FunctionDef | `test_demand_ceilings_cap800` | 39 | docstringなし（下のコード参照） |
| FunctionDef | `test_appendix_a3_regression` | 59 | docstringなし（下のコード参照） |
| FunctionDef | `test_plateau_800_vs_1500` | 67 | #11: 能力800で台地1点、能力1500で台地28点（退化の再現）。 |
| FunctionDef | `test_base_allocation_fxb` | 76 | docstringなし（下のコード参照） |
| FunctionDef | `test_profit_identity` | 84 | 損益恒等式：profit = rev − cost（残差ゼロ）。 |
| FunctionDef | `_legacy_simplex_grid` | 96 | Phase 6-2 以前の実装（順序の参照用・改変禁止）。 |
| FunctionDef | `test_simplex_grid_3d_order_unchanged` | 103 | 231点が値も順序も Phase 6-2 以前の実装と完全一致すること（C8・最重要）。 |
| FunctionDef | `test_simplex_grid_point_counts` | 110 | docstringなし（下のコード参照） |
| FunctionDef | `test_simplex_grid_sums_to_one` | 118 | docstringなし（下のコード参照） |
| FunctionDef | `test_markets_order_is_deterministic` | 126 | docstringなし（下のコード参照） |
| FunctionDef | `test_markets_order_not_alphabetical` | 133 | docstringなし（下のコード参照） |
| FunctionDef | `_dummy_blocks` | 140 | docstringなし（下のコード参照） |
| FunctionDef | `test_scan_surface_guard_raises` | 147 | N=7（230,230点）は simplex_grid() を呼ぶ前に ValueError（C9）。N=6 は通る。 |
