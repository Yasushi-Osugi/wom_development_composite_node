...
tags: [wom, code]
---
# tests/test_allocation_nonconcave.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_allocation_nonconcave.py) · [原文テキスト](../../90_Raw/tests/test_allocation_nonconcave.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `soysauce_blocks` | 26 | s1_base 相当（cliff未設定）。Phase 4 の回帰値と一致するはず。 |
| FunctionDef | `s9_blocks` | 32 | s9_fta_cliff（US: 12.5%→0%、25,000lot以上で発動）。cap_wk=500 前提。 |
| FunctionDef | `test_tariff_at_returns_base_when_no_cliff` | 47 | tariff_rate_preferential=None なら tariff_at(qty) が常に tariff_rate |
| FunctionDef | `test_unit_pnl_at_quantity_matches_unit_pnl_without_cliff` | 56 | cliff なしで unit_pnl_at_quantity() と unit_pnl() が完全一致 |
| FunctionDef | `test_phase4_regression_unchanged` | 67 | soysauce s1_base / cap_wk=800 で Phase 4 の回帰値が1円も変わらない |
| FunctionDef | `test_evaluate_point_unchanged_without_cliff` | 86 | evaluate_point() の改修後も cliff なしの結果が全231点で不変 |
| FunctionDef | `test_tariff_at_switches_at_threshold` | 107 | qty が閾値の前後で税率が切り替わる（境界値 qty==T は特恵側） |
| FunctionDef | `test_preferential_flips_ranking` | 119 | US に特恵が発動すると単価 2,077.5 > EU 1,831.5 で順位が逆転する |
| FunctionDef | `test_merit_order_is_myopic` | 129 | ①が閾値を先読みしない（s9_fta_cliff で US=16,825 に留まり triggered=False） |
| FunctionDef | `test_merit_order_profit_uses_effective_margin` | 149 | 利益計算は実配分量に応じた単価を使う（順位付けの単価ではない） |
| FunctionDef | `test_s9_cliff_regression` | 182 | s9_fta_cliff / cap_wk=500 で ① 93,824,700 JPY、格子最適 103,552,800 JPY、 |
| FunctionDef | `test_structural_residual_is_negative` | 196 | residual ≈ -9,728,100 で負、attributable_to_grid_resolution が False |
| FunctionDef | `test_residual_underestimates_true_gap` | 210 | 真の連続最適 103,891,296.0（Phase 6-1 で true_continuous_optimum() が実計算した |
| FunctionDef | `test_scenario_csv_loads_cliff_columns` | 235 | s9_fta_cliff の US 行から特恵税率と閾値が読める |
| FunctionDef | `test_scenario_csv_missing_columns_ok` | 248 | 2列が存在しない古い CSV でも例外にならない |
| FunctionDef | `test_true_optimum_s9_regression` | 270 | s9_fta_cliff の真の連続最適 103,891,296.0 JPY（Phase 6-1 で実装確定した回帰値）。 |
| FunctionDef | `test_true_optimum_case_a_matches_greedy` | 285 | ケース S={}（US が特恵未発動）を単独で解いた利益が、①メリットオーダーの |
| FunctionDef | `test_true_optimum_respects_threshold_lower_bound` | 300 | 閾値制約が binding になる合成ケースで、下界制約を持たない素朴な解法 |
| FunctionDef | `test_true_optimum_equals_greedy_when_linear` | 341 | cliff が無ければ真の連続最適は貪欲法の解と厳密一致する |
| FunctionDef | `test_true_optimum_is_upper_bound` | 355 | 真の連続最適は、格子最適・貪欲法の利益のいずれも下回らない |
| FunctionDef | `test_compare_with_grid_backward_compatible` | 379 | true_optimum を渡さない compare_with_grid() の返却が Phase 5 と完全に同一 |
| FunctionDef | `test_residual_coverage_none_when_linear` | 401 | s1_base（線形）で structural_optimality_gap==0 のとき residual_coverage は |
