...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_merit_order.py) · [原文テキスト](../../90_Raw/tests/test_merit_order.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `sample_suppliers` | 18 | テスト用サンプルサプライヤー |
| ClassDef | `TestMeritOrderAnalyzerValidation` | 64 | バリデーション機能のテスト |
| ClassDef | `TestMeritOrderCalculation` | 137 | Merit Order計算のテスト |
| ClassDef | `TestExchangeRateHandling` | 222 | 為替レート処理のテスト |
| ClassDef | `TestAverageCalculations` | 259 | 平均値計算のテスト |
| ClassDef | `TestSingleSourceConstraint` | 285 | 単一ソース制約のテスト |
| ClassDef | `TestEndToEnd` | 313 | エンドツーエンドのテスト |
| ClassDef | `TestHelperFunctions` | 349 | ヘルパー関数のテスト |
| ClassDef | `TestExportToJSON` | 390 | JSON エクスポート機能のテスト |
| ClassDef | `TestExchangeRateValidation` | 436 | 為替レート検証のテスト |
| FunctionDef | `test_valid_suppliers` | 67 | 正常なサプライヤーデータの検証 |
| FunctionDef | `test_missing_required_field` | 73 | 必須フィールド欠落時の検証 |
| FunctionDef | `test_negative_unit_cost` | 87 | 負のコストの検証 |
| FunctionDef | `test_quality_score_out_of_range` | 101 | 品質スコア範囲外の検証 |
| FunctionDef | `test_duplicate_supplier_id` | 115 | 重複したsupplier_idの検証 |
| FunctionDef | `test_basic_merit_order_generation` | 140 | 基本的なMerit Order生成 |
| FunctionDef | `test_merit_order_sorted_by_cost` | 154 | Merit Orderがコスト順にソートされていることを確認 |
| FunctionDef | `test_demand_allocation` | 166 | 需要が正しく配分されたことを確認 |
| FunctionDef | `test_total_cost_calculation` | 180 | 総コスト計算の正確性 |
| FunctionDef | `test_quality_constraint` | 195 | 品質制約が機能していることを確認 |
| FunctionDef | `test_lead_time_constraint` | 208 | リードタイム制約が機能していることを確認 |
| FunctionDef | `test_exchange_rate_conversion` | 225 | 為替レート変換の正確性 |
| FunctionDef | `test_average_quality` | 262 | 平均品質スコアの計算 |
| FunctionDef | `test_average_lead_time` | 273 | 平均リードタイムの計算 |
| FunctionDef | `test_single_source_max_constraint` | 288 | 1社からの調達比率上限の制約 |
| FunctionDef | `test_full_workflow` | 316 | 全体のワークフロー |
| FunctionDef | `test_load_suppliers_csv_file_not_found` | 352 | ファイルが存在しない場合 |
| FunctionDef | `test_load_suppliers_csv_invalid_format` | 357 | CSV 形式が不正な場合（未終端のクォートでトークナイズエラーを誘発） |
| FunctionDef | `test_load_suppliers_csv_missing_columns` | 370 | 必須カラムが足りない場合 |
| FunctionDef | `test_load_suppliers_csv_empty_file` | 378 | ファイルが空の場合 |
| FunctionDef | `test_export_to_json_success` | 393 | 正常な出力 |
| FunctionDef | `test_export_to_json_invalid_path` | 410 | 出力パスが不正な場合 |
| FunctionDef | `test_export_to_json_non_serializable` | 421 | シリアライズ不可のデータが含まれている場合 |
| FunctionDef | `test_exchange_rate_zero` | 439 | 為替レートが 0 の場合 |
| FunctionDef | `test_exchange_rate_negative` | 456 | 為替レートが負の場合 |
| FunctionDef | `test_exchange_rate_nan` | 472 | 為替レートが NaN の場合 |
| FunctionDef | `test_exchange_rate_string` | 488 | 為替レートが文字列の場合 |
| FunctionDef | `test_exchange_rate_valid` | 505 | 為替レートが有効な場合 |
