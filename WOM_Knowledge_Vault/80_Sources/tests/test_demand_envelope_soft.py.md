...
tags: [wom, code]
---
# tests/test_demand_envelope_soft.py

静的コード資料（実行検証ではない）。

[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_demand_envelope_soft.py) · [原文テキスト](../../90_Raw/tests/test_demand_envelope_soft.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_demo_mom` | 28 | docstringなし（下のコード参照） |
| FunctionDef | `test_soft_mode_levels_at_cap_soft` | 43 | docstringなし（下のコード参照） |
| FunctionDef | `test_hard_mode_unchanged_cap_soft_is_flag_only` | 69 | docstringなし（下のコード参照） |
| FunctionDef | `test_soft_mode_holiday_spreads_at_cap_soft` | 91 | docstringなし（下のコード参照） |
