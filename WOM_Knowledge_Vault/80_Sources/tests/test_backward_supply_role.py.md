...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_backward_supply_role.py) · [原文テキスト](../../90_Raw/tests/test_backward_supply_role.py.txt)
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_build_tree` | 57 | Build a minimal InBound-only tree: one MOM root with children named |
| FunctionDef | `_set_demand` | 90 | docstringなし（下のコード参照） |
| FunctionDef | `_run_in_propagate` | 95 | docstringなし（下のコード参照） |
| FunctionDef | `test_supply_role_read_from_csv_and_stored_on_plan_node` | 111 | docstringなし（下のコード参照） |
| FunctionDef | `test_supply_role_blank_defaults_to_assembly` | 129 | docstringなし（下のコード参照） |
| FunctionDef | `test_supply_role_unknown_value_defaults_to_assembly` | 142 | docstringなし（下のコード参照） |
| FunctionDef | `test_confluence_two_children_split_evenly_no_duplication_no_loss` | 159 | docstringなし（下のコード参照） |
| FunctionDef | `test_confluence_uneven_split_uses_remainder` | 181 | docstringなし（下のコード参照） |
| FunctionDef | `test_assembly_two_children_each_get_full_copy` | 197 | docstringなし（下のコード参照） |
| FunctionDef | `test_mixed_confluence_and_assembly_siblings` | 212 | docstringなし（下のコード参照） |
| FunctionDef | `test_single_child_confluence_gets_full_demand` | 233 | docstringなし（下のコード参照） |
| FunctionDef | `test_single_child_assembly_gets_full_demand` | 242 | docstringなし（下のコード参照） |
