...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tests/test_planning_state.py) · [原文テキスト](../../90_Raw/tests/test_planning_state.py.txt)

基準: `7d6c7d734ebdcb7b213bab3116ca59d3d4935f55`。[[00_Start/Source_Policy|出典と読み方]]

関連機能: [[10_Functions/02_Demand_Allocation|Optimization on Demand Allocation]] / [[10_Functions/03_Planning|Planning]]
...
| 種別 | 名前 | 行 | 説明の先頭行 |
|---|---|---:|---|
| FunctionDef | `_copy_model` | 36 | docstringなし（下のコード参照） |
| FunctionDef | `test_golden_unchanged_by_default` | 48 | docstringなし（下のコード参照） |
| FunctionDef | `test_allocation_conserves_demand` | 71 | docstringなし（下のコード参照） |
| FunctionDef | `test_allocation_rounding_is_exact` | 91 | docstringなし（下のコード参照） |
| FunctionDef | `test_zero_weeks_preserved` | 118 | docstringなし（下のコード参照） |
| FunctionDef | `test_state_transitions` | 148 | docstringなし（下のコード参照） |
| FunctionDef | `test_planning_loop_closes` | 199 | docstringなし（下のコード参照） |
| FunctionDef | `test_warmup_respects_demand_file` | 233 | docstringなし（下のコード参照） |
| FunctionDef | `closed_loop_state` | 263 | soysauce s1_base / cap_wk=800 / 配分 (0.10,0.45,0.45) で1周させた最終 state。 |
| FunctionDef | `test_plan_eval_uses_chosen_allocation` | 284 | A5.1: --allocation を明示したとき、plan_eval.profit は P_opt ではなく |
| FunctionDef | `test_gap_decomposition_identity` | 296 | A5.2（最重要）: total == quantity + fx_assumption + residual（±1円）。 |
| FunctionDef | `test_no_gap_vs_plan_pct_key` | 306 | A5.3: realized に gap_vs_plan_pct キーが存在しないこと（null でも残さない）。 |
| FunctionDef | `test_reversal_populated` | 311 | A5.5: soysauce（fx_usd=150）で reversal.boundary が 117.0 または 119.0。 |
| FunctionDef | `test_fx_effective_is_quantity_weighted` | 319 | A5.4: 出荷が偏る合成ケースで、単純平均とは異なる値になること。 |
