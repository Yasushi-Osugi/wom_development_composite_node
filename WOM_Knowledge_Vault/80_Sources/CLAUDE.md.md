...
[GitHub原本・固定SHA](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/CLAUDE.md) · [原文テキスト](../90_Raw/CLAUDE.md.txt)
...
- 未対応・Phase 6送り
- Phase 6-1：`true_optimum`の実計算化（**実装完了 2026-09-11、377件全PASS**）
- 以下は実装前の方針メモ（2026-09-09、記録として保存）
...
s9_fta_cliff / cap_wk=500（非凹性の検証）:
  ...
  （真の連続最適（Phase 6-1 実装で確定）: profit=103,891,296、構造由来の取りこぼし10,066,596のうち
   |residual|が96.6%を捉える＝下界として機能）
```
...
**`compare_with_grid()`自体の実装変更は不要だった**——判定式`abs(structural_residual) <= abs_tol`は符号によらず正しく動作していた。訂正したのは意味づけ（ドキュメント）のみ。非凹ケースでは格子最適点が能力を使い切り（`grid_idle=0`）`expected_gap=0`となるため、`|residual|`は構造由来の乖離を過小評価する（この例では96.6%）——「厳密な分解」ではなく「符号を非凹性の指標、絶対値を下界」として使う設計。
...
- `true_optimum`のハードコード解消（下記メモ参照）

### Phase 6-1：`true_optimum`の実計算化（**実装完了 2026-09-11、377件全PASS**）

**実装結果（2026-09-11）**：`wom/allocation/merit_order.py`に`true_continuous_optimum()`を新規実装。cliffのon/offを全列挙（2^|K|ケース）し、各ケースを**閾値を配分量の上下界として制約に持たせた**線形問題として貪欲法で厳密に解く。`compare_with_grid()`に`true_optimum`引数と4フィールド（`true_optimum`/`structural_optimality_gap`/`grid_resolution_error`/`residual_coverage`）を追加（既存キーは不変、引数省略時の返却は従来と完全同一）。テスト7件追加で**377件全PASS**、golden 13ケース不変。設計正典は`requests/Phase6_DesignMD_NMarketHierarchy.md`§3（rev.2）＋`requests/Phase6-1_RequestLetter_to_CodeKun.md`。

**確定した回帰値（s9_fta_cliff）**：`true_optimum=103,891,296.0`（配分 JP=0/US=35,176/EU=16,824）／`structural_optimality_gap=10,066,596`／`grid_resolution_error=338,496`／`residual_coverage=0.966`。恒等式`9,728,100 + 338,496 = 10,066,596`。`s1_base`（線形）では`true_optimum == P_greedy == 135,529,822.5`が厳密成立、`structural_optimality_gap=0`、`residual_coverage=None`。

**⚠️ 設計書rev.1の手計算値103,881,758は誤りだった（重要な記録）**：真の最適としていた配分`JP=7/US=35,168/EU=16,825`は最適点ではない。単位マージン2,077.5のUS需要を8 lot残したまま、最低マージンのJP（750）に7 lot配っており厳密に劣る。差は1円まで分解できる——`US +8×2,077.5 = +16,620` / `EU −1×1,831.5 = −1,831.5` / `JP −7×750 = −5,250` → **+9,538.5**。**丸め誤差ではなく手計算側の最適化の誤り**であり、実装がそれを検出した。Request Letterの申し送り「手計算値に合わせて実装を調整しない」が機能した事例。公開済みnote記事「利益地形図で読み解く事業計画」の該当数値も同時に訂正した。

**§3.3の設計変更（記録）**：設計書rev.1は「税率を固定して解いてから仮定と矛盾しないか検査する」方式だったが、これは**閾値がbindingな解を取り逃す**（「USが特恵を発動」ケースの最適はしばしば「USをちょうど閾値まで積む」点にあり、素朴な貪欲では閾値未満で止まって棄却されてしまう）。Request Letter V1.2で**閾値を上下界の制約として先に持たせる**方式に差し替え、設計書§3.3もrev.2で全面改訂。この方式では仮定と結果が構造的に矛盾し得ないため、事後の整合性チェック自体が不要になる。

---

#### 以下は実装前の方針メモ（2026-09-09、記録として保存）

`tools/demo_allocation_nonconcave.py`の`true_optimum = 103_891_296.0`は設計書§2.3の**手計算値**であり、実行時には計算していない（コメントにも明記済み）。`ev-thailand-2026`（LC率閾値で非凹）にPhase 6で適用する際、この値を**実計算に置き換える**方針を大杉さんと確認。以下はその実装方針（Code君案、大杉さんとの対話で確定）。

**範囲・対象・目的関数**：新しい問題設定は不要。`evaluate_point()`/`build_allocation_merit_order()`が既に評価しているものと完全に同一——目的関数は粗利`profit(x)=Σ_m q_m×margin_m(q_m)`、変数は配分比率`(x_JP,x_US,x_EU)`（単体制約`Σx=1,x≥0`）、制約は需要天井`q_m=min(x_m·cap, demand_qty[m])`。「最適化計算する」とは、この同じ関数をδ=0.05の231点ではなく**連続領域で厳密に最大化する**という意味に過ぎない。

**解法（LPソルバー不使用、既存スタイルを維持）**：
- 非cliff市場（単価が`q`に依存しない）は、限界価値降順のメリットオーダー貪欲が厳密最適（fractional knapsack定理）——Phase 4までの世界そのもの。
- cliff市場は「閾値未満」「閾値以上」の**2値でしか不連続が起きない**ため、cliff市場ごとに2ケースへ分岐し、各ケース内は同じ貪欲法で解ける：
  - **ケースA（閾値未満）**：ranking単価のまま、`q_cliff市場 ≤ threshold`を制約に追加してメリットオーダー貪欲
  - **ケースB（閾値以上）**：`q_cliff市場 ≥ threshold`を先に確保（floor割当）してから、残り容量で（effective単価を使い）メリットオーダー貪欲
  - `true_optimum = max(全ケースの利益)`。cliff市場がN個なら2^N ケース（`ev-thailand-2026`は基本1閾値の見込みなのでN=1〜2程度）
...
**検算（手計算・実装前の裏付け）**：soysauceのs9_fta_cliffで上記方式を手で計算したところ、ケースA=35,175×1831.5+16,825×1747.5=**93,824,700.0**（①の実測値と厳密一致）、ケースB≈35,176×2077.5+16,824×1831.5≈**103.89M**（**この概算のほうが正しかった**——実装は 103,891,296.0 を返した。設計書rev.1の103,881,758は最適でない配分の利益だったことが後日判明）。実装はこの方式のコード化で足りた。

**フィールド名（2026-09-09追記）**：Phase 6で`compare_with_grid()`に実装する際、`true_optimum − mo["profit"]`（真の連続最適−貪欲解）の値は`structural_optimality_gap`という名前で返す。既存の`structural_residual`（`gap_amt − expected_gap`、実装が返す下界）と対になる関係——`structural_residual ≤ structural_optimality_gap`（今回の実測では9,728,100 ≤ 10,066,596）——であることをフィールド名からも読み取れるようにする。
