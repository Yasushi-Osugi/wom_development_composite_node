...
**N市場化（Phase 6）** — δ=0.05 の格子点数は市場数 N に対して `C(20+N-1, N-1)` で増え、N=21（`oil-global-2027` の市場数）では約1,378億点となって grid scan そのものが破綻します。市場を3つずつのグループに束ねる**階層化単体格子**なら三角図13枚 × 231点 = 3,003点に収まる見込みで、その近似誤差は `structural_optimality_gap` で金額として測れます。
