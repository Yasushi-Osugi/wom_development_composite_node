---
function_id: M22S
level: 4
tags: [wom, function, L4]
---
# Planning実行と結果確認の設計

**役割：** 採用案を供給網で実行可能か検証する。

**状態：** 関連実装あり・静的確認。以下は固定SHAの関連実装と設計資料を業務機能単位に再編集した設計読解ノート。新しいcore仕様の承認ではない。

## 入力と前提

採用配分、週次入力、能力、原価前提。

## 処理規則と責任境界

配分から需要を作りPlanning/PPCを実行し、S3で結果を確認する。pre_planとfeasible_planを区別する。

## 出力と利用先

実行済み状態と能力・供給診断。 上位の詳細機能へ戻り、その機能が属する業務領域から利用先を確認する。機能間の受渡し全体は「業務フローとインターフェース」に別記する。

## 確認例

採用数量に対して実出荷が不足した際、配分結果だけで成功判定しない。

この例は仕様理解・今後の確認に使う観点であり、本Vault作成時に新しく実行したテスト結果ではない。

## 例外・限界・未確定事項

現在の結果確認範囲と将来S4機能を分ける。

## 設計・コードの根拠

- [wom/cockpit/s3_run.py](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/cockpit/s3_run.py)
- [wom/planning_state/__init__.py](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/planning_state/__init__.py)
- [tools/run_planning_loop.py](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/tools/run_planning_loop.py)

## 業務思想・沿革の参考

- [記事08：週次で動く世界＝経営とオペレーションをつなぐWeekly Operation Model（WOM）](https://note.com/osuosu1123/n/nc76451054220)
- [記事09：Weekly Operation Model（WOM）をPythonで実装した——週次サプライチェーン教材の最小構成](https://note.com/osuosu1123/n/n37e8b53c5b07)
- [記事12：AIでサプライチェーンを可視化（第８回：「利益の空白」を埋める）](https://note.com/osuosu1123/n/nde1d9b686a6e)

記事は業務背景の参考。現行実装の存在は上のコード・設計と区別して判断する。記事の短い要約と適用上の注意は「記事と機能の対応表」を参照。

## 上位機能

[[01_Functions/L3/M22 Planning実行と結果確認|Planning実行と結果確認]]
