---
function_id: A31S
level: 4
tags: [wom, function, L4]
---
# 配分の感度と頑健性の分析の設計

**役割：** 最良点が条件変更でどれほど変わるかを見る。

**状態：** 関連実装あり・静的確認。以下は固定SHAの関連実装と設計資料を業務機能単位に再編集した設計読解ノート。新しいcore仕様の承認ではない。

## 入力と前提

複数シナリオ、配分案、為替・原料価格の範囲。

## 処理規則と責任境界

市場順位の切替、相互作用、頑健候補や制約の機会費用を分析する。

## 出力と利用先

感度・切替点・比較指標。 上位の詳細機能へ戻り、その機能が属する業務領域から利用先を確認する。機能間の受渡し全体は「業務フローとインターフェース」に別記する。

## 確認例

一つの価格条件での最良点と、複数条件に対して安定した案を区別する。

この例は仕様理解・今後の確認に使う観点であり、本Vault作成時に新しく実行したテスト結果ではない。

## 例外・限界・未確定事項

確率分布や将来価格予測の正しさを保証する機能ではない。

## 設計・コードの根拠

- [wom/allocation/analytics.py](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/allocation/analytics.py)
- [wom/allocation/regime_map.py](https://github.com/Yasushi-Osugi/wom_development_composite_node/blob/7d6c7d734ebdcb7b213bab3116ca59d3d4935f55/wom/allocation/regime_map.py)

## 業務思想・沿革の参考

- [記事05：価格=価値X制約、コスト=構造、利益=設計の成果](https://note.com/osuosu1123/n/nef9e485ce0be)
- [記事12：AIでサプライチェーンを可視化（第８回：「利益の空白」を埋める）](https://note.com/osuosu1123/n/nde1d9b686a6e)

記事は業務背景の参考。現行実装の存在は上のコード・設計と区別して判断する。記事の短い要約と適用上の注意は「記事と機能の対応表」を参照。

## 上位機能

[[01_Functions/L3/A31 配分の感度と頑健性の分析|配分の感度と頑健性の分析]]
