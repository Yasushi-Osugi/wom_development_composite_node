...
### Phase 6 メモ：`true_optimum`ハードコード解消の方針（構想のみ・実装なし、2026-09-09）

`tools/demo_allocation_nonconcave.py`の`true_optimum = 103_881_758.0`は設計書§2.3の**手計算値**であり、実行時には計算していない（コメントにも明記済み）。`ev-thailand-2026`（LC率閾値で非凹）にPhase 6で適用する際、この値を**実計算に置き換える**方針を大杉さんと確認。以下はその実装方針（Code君案、大杉さんとの対話で確定）。
