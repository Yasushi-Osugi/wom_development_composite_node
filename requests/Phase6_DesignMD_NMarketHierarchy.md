...
階層化（483点）の最良点        =  P_hier
階層化の誤差                   =  P_flat − P_hier     ← 金額で出る。0 以上
```
...
- [ ] `oil-global-2027`（21市場）で階層化スキャンが **13ノード・1,743点**で完走する（→ **6-3b**。`ga_market_aggregation.csv` の作成が前提）
...
### 9.2 未決（本 rev.4 で新たに出たもの）

**R5. `oil-global-2027` の `ga_market_aggregation.csv` を 6-3 のスコープに入れるか。**
rev.4 の調査で、`derive_cost_blocks()` が要求する9本の CSV のうち、`oil-global-2027` に**無いのは `ga_market_aggregation.csv` 1本だけ**であることが確認できた（他8本は揃っている）。この1本が無いと、A系統は 21市場ケースに接続できず、§8 の「1,743点で完走する」も検証できない。

内容は `sc_tree_master.csv` の `leaf_out` 21件と `node_master.csv` の `region` から機械的に書ける（§5.2 の木がそのまま表になる）ため、作業量は大きくない。**6-3 の最後の小ステップ（6-3b）として含めることを推奨する。** 21市場で実際に走って初めて「1,378億点 → 1,743点」が主張になるためである。

分ける場合は Phase 6-4 とし、6-3 の完了条件から §8 の当該行を外す。

**R6. 階層化で内部比率が最適化されることを、経営的にどう位置づけるか。**
§5.5(2) の区別——3市場スキャンは `internal_ratio` を固定、6地域の階層スキャンはそれを最適化する——は、実務的には「地域内の配分（US_W と US_E の比率）まで意思決定の対象にするか」という問いになる。仕様書 §2.4 は現状これを「最適化対象外」としている。**階層化はこの前提を変えうる**ので、そのままでよいか確認したい。
